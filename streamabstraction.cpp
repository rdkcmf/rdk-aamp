...
void MediaTrack::StartPlaylistDownloaderThread()
{
	...
	if(!playlistDownloaderThreadStarted)
	{
		...
		if(NULL == playlistDownloaderThread)
		{
			...
			AAMPLOG_INFO("Thread created for PlaylistDownloader [%u]", playlistDownloaderThread->get_id());
		}
		else
		{
...
void MediaTrack::StartInjectLoop()
{
	...
	try
	{
		...
		AAMPLOG_INFO("Thread created for RunInjectLoop [%u]", fragmentInjectorThreadID.get_id());
	}
...
void MediaTrack::StartInjectChunkLoop()
{
	...
	try
	{
		...
		AAMPLOG_INFO("Thread created for RunInjectChunkLoop [%u]", fragmentChunkInjectorThreadID.get_id());
	}
	catch(const std::exception& e)
	{
...
void MediaTrack::RunInjectLoop()
{
...

    {
		try
		{
			...
			AAMPLOG_INFO("Thread created for MonitorBufferHealth [%u]", bufferMonitorThreadID.get_id());
