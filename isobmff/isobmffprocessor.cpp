...
{
	AAMPLOG_WARN(p_aamp->mConfig, "IsoBmffProcessor: Created IsoBmffProcessor(%p) for type:%d and peerProcessor(%p)", this, type, peerBmffProcessor);
	if (peerProcessor)
	{
...
bool IsoBmffProcessor::sendSegment(char *segment, size_t& size, double position, double duration, bool discontinuous, bool &ptsError)
{
	...
	AAMPLOG_TRACE(p_aamp->mConfig, "IsoBmffProcessor: [%s] sending segment at pos:%f dur:%f", IsoBmffProcessorTypeName[type], position, duration);
	...
	if (type == eBMFFPROCESSOR_TYPE_AUDIO)
	{
		if (!processPTSComplete)
		{
			...
			else
			{
				...
				if (!processPTSComplete)
				{
					AAMPLOG_INFO(p_aamp->mConfig, "IsoBmffProcessor:[%s] Going into wait for PTS processing to complete", IsoBmffProcessorTypeName[type]);
	...
	if (ret && !processPTSComplete && playRate == AAMP_NORMAL_PLAY_RATE)
	{
		...
		if (buffer.isInitSegment())
		{
			...
			{
				timeScale = tScale;
				AAMPLOG_INFO(p_aamp->mConfig, "IsoBmffProcessor:[%s] TimeScale (%ld) set", IsoBmffProcessorTypeName[type], timeScale);
			}
		...
		else
		{
			...
			if (buffer.getFirstPTS(fPts))
			{
				...
				AAMPLOG_WARN(p_aamp->mConfig, "IsoBmffProcessor: [%s] Base PTS (%lld) set", IsoBmffProcessorTypeName[type], basePTS);
			}
			else
			{
				AAMPLOG_ERR(p_aamp->mConfig, "IsoBmffProcessor: [%s] Failed to process pts from buffer at pos:%f and dur:%f", IsoBmffProcessorTypeName[type], position, duration);
			}
			...
			if (ret && processPTSComplete)
			{
				if (timeScale == 0)
				{
					if (initSegment.empty())
					{
						AAMPLOG_WARN(p_aamp->mConfig, "IsoBmffProcessor: [%s] Init segment missing during PTS processing!", IsoBmffProcessorTypeName[type]);
						p_aamp->SendErrorEvent(AAMP_TUNE_MP4_INIT_FRAGMENT_MISSING);
						ret = false;
					}
					else
					{
						AAMPLOG_WARN(p_aamp->mConfig, "IsoBmffProcessor: [%s] MDHD/MVHD boxes are missing in init segment!", IsoBmffProcessorTypeName[type]);
						uint32_t tScale = 0;
						if (buffer.getTimeScale(tScale))
						{
							timeScale = tScale;
						...
						if (timeScale == 0)
						{
							AAMPLOG_ERR(p_aamp->mConfig, "IsoBmffProcessor: [%s] TimeScale value missing in init segment and mp4 fragment, setting to a default of 1!", IsoBmffProcessorTypeName[type]);
...
{
	AAMPLOG_WARN(p_aamp->mConfig, "[%s] Base PTS (%lld) and TimeScale (%ld) set", IsoBmffProcessorTypeName[type], pts, tScale);
	pthread_mutex_lock(&m_mutex);
	basePTS = pts;
	timeScale = tScale;
	processPTSComplete = true;
...
void IsoBmffProcessor::cacheInitSegment(char *segment, size_t size)
{
	...
	AAMPLOG_INFO(p_aamp->mConfig, "IsoBmffProcessor: [%s] Caching init fragment", IsoBmffProcessorTypeName[type]);
...
void IsoBmffProcessor::pushInitSegment(double position)
{
	...
	AAMPLOG_WARN(p_aamp->mConfig, "IsoBmffProcessor: [%s] Push init fragment", IsoBmffProcessorTypeName[type]);
