...
static MediaTypeMapTelemetry mediaTypeMapTelemetry[] =
{
	{eMEDIATYPE_TELEMETRY_AVS, "avs"},
	{eMEDIATYPE_TELEMETRY_DRM, "drm"},
	{eMEDIATYPE_TELEMETRY_INIT, "ini"},
	{eMEDIATYPE_TELEMETRY_MANIFEST, "man"},
	{eMEDIATYPE_TELEMETRY_UNKNOWN, "unknown"}
};
...
//				InterruptableMsSleep( 250 ); // this can be uncommented to locally induce extra per-download latency
				...
				if (gpGlobalConfig->logging.isLogLevelAllowed(reqEndLogLevel))
				{
					...
					AAMPLOG(reqEndLogLevel, "HttpRequestEnd: {\"url\":\"%.500s\",\"curlTime\":%2.4f,\"times\":{\"total\":%2.4f,\"connect\":%2.4f,\"startTransfer\":%2.4f,\"resolve\":%2.4f,\"appConnect\":%2.4f,\"preTransfer\":%2.4f,\"redirect\":%2.4f,\"dlSz\":%g,\"ulSz\":%ld}%s%s,\"responseCode\":%ld,\"type\":%d,\"class\":%.3s\"}",
						...
						totalPerformRequest,
						total, connect, startTransfer, resolve, appConnect, preTransfer, redirect, dlSize, reqSize,
						appName.c_str(), timeoutClass.c_str(), http_code, simType, mediaTypeMapTelemetry[mediaType].description);
				}
	...
	{ // ignore comments
		...
		else if (cfg.at(0) == '*')
		{
			...
			if (pos != std::string::npos)
			{
				...
				logprintf("aamp override:%s", cfg.c_str());
 ...
 */
PlayerInstanceAAMP::PlayerInstanceAAMP(StreamSink* streamSink, Playermode playermode) : aamp(NULL), mInternalStreamSink(NULL), mJSBinding_DL()
{
