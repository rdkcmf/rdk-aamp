...
void PrivateInstanceAAMP::Tune(const char *mainManifestUrl, const char *contentType, bool bFirstAttempt, bool bFinalAttempt,const char *pTraceID)
{
	...
	{
		logprintf("aamp_tune: TAG fr_test_fed_1 attempt: %d format: %s URL: %s", mTuneAttempts, mIsDash?"DASH":"HLS", mManifestUrl.c_str());
	}
	else
	{
		...
		for (int i = 0; i < AAMP_MAX_SIMULTANEOUS_INSTANCES; i++)
		{
			if (this == gActivePrivAAMPs[i].pAAMP)
			{
				...
				else
				{
					...
					{
						logprintf("PrivateInstanceAAMP::%s:%d: Schedule Retune errorType %d", __FUNCTION__, __LINE__, errorType);
						gActivePrivAAMPs[i].reTune = true;
						g_idle_add(PrivateInstanceAAMP_Retune, (gpointer) this);
					}
				}
