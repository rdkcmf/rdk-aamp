...
static void ProcessConfigEntry(std::string cfg)
{
	...
	{ // ignore comments
		...
		else if (ReadConfigNumericHelper(cfg, "ad-position=", gpGlobalConfig->adPositionSec) == 1)
		{
		...
		else if(ReadConfigNumericHelper(cfg, "dash-max-drm-sessions=", gpGlobalConfig->dash_MaxDRMSessions) == 1)
		{
			// Read value in KB , convert it to bytes
...
PrivateInstanceAAMP::PrivateInstanceAAMP() : mCurrentLanguageIndex(0),mVideoEnd(NULL),mTimeToTopProfile(0),mTimeAtTopProfile(0),mPlaybackDuration(0),mTraceUUID(),
	mAbrBitrateData(), mLock(), mMutexAttr(),
	...
	mManifestUrl(""), mTunedManifestUrl(""), mAdUrl(""),
