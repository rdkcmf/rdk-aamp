...
static void ProcessConfigEntry(char *cfg)
{
	...
	{ // ignore comments
		...
		else if(sscanf(cfg, "max-playlist-cache=%d", &gpGlobalConfig->gMaxPlaylistCacheSize) == 1)
		{
...
PrivateInstanceAAMP::PrivateInstanceAAMP() : mAbrBitrateData(), mLock(), mMutexAttr(),
	...
	mTuneType(eTUNETYPE_NEW_NORMAL), m_fd(-1), mIsLive(false), mTuneCompleted(false), mFirstTune(true), mfirstTuneFmt(-1), mTuneAttempts(0), mPlayerLoadTime(0),
