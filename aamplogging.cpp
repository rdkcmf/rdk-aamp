 ...
 * @retuen void
 */
void logprintf_cfg(int playerId, const char *function, int line, const char *format, ...)
{
        int len = 0;
        va_list args;
        ...
        len = sprintf(gDebugPrintBuffer, "[AAMP-PLAYER]%s:%d:%d ", function, line, playerId);
 ...
 * @retuen void
 */
void logprintf_gp(const char *function, int line, const char *format, ...)
{
        int len = 0;
        va_list args;
        va_start(args, format);
        ...
        char gDebugPrintBuffer[MAX_DEBUG_LOG_BUFF_SIZE];
        len = sprintf(gDebugPrintBuffer, "[AAMP-PLAYER]%s:%d: ", function, line);
        vsnprintf(gDebugPrintBuffer+len, MAX_DEBUG_LOG_BUFF_SIZE-len, format, args);
        gDebugPrintBuffer[(MAX_DEBUG_LOG_BUFF_SIZE-1)] = 0;
