				...
				if (IS_PES_PACKET_START(pesStart))
				{
					if (PES_OPTIONAL_HEADER_PRESENT(pesStart))
					{
						if ((pesStart[7] & 0x80) && ((pesStart[9] & 0x20) == 0x20))
						{
							...
							if( (timeStamp<base_pts) != (current_pts<base_pts) )
							{
								logprintf( "PTS rollover");
							}
...
#if !defined(REALTEKCE) // TODO: remove and retest across platforms
...
TSProcessor::TSProcessor(class PrivateInstanceAAMP *aamp,StreamOperation streamOperation, int track, TSProcessor* peerTSProcessor, TSProcessor* auxTSProcessor)
	...
	m_auxTSProcessor(auxTSProcessor), m_auxiliaryAudio(false), m_AudioTrackIndexToPlay(0)
{
