		...
		if (old_state == GST_STATE_NULL && new_state == GST_STATE_READY)
		{
			...
			if (AAMPGstPlayer_isVideoOrAudioDecoder(GST_OBJECT_NAME(msg->src), _this))
			{
				if (AAMPGstPlayer_isVideoDecoder(GST_OBJECT_NAME(msg->src), _this))
				{
					...
					_this->privateContext->video_dec_handler_id = g_signal_connect(_this->privateContext->video_dec, "first-video-frame-callback", G_CALLBACK(AAMPGstPlayer_OnFirstVideoFrameCallback), _this);
				}
				else
				{
