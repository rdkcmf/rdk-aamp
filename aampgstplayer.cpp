...
static void AAMPGstPlayer_SendPendingEvents(PrivateInstanceAAMP *aamp, AAMPGstPlayerPriv *privateContext, MediaType mediaType, GstClockTime pts)
{
	...
	if (stream->format == FORMAT_ISO_BMFF)
	{
		...
		if (enableOverride == FALSE && aamp->IsHlsMp4Asset())
		{
			GstStructure * segmentOverride = gst_structure_new("aamp-segment-override", "enable", G_TYPE_BOOLEAN, TRUE, NULL);
			if (!gst_pad_push_event(sourceEleSrcPad, gst_event_new_custom(GST_EVENT_CUSTOM_DOWNSTREAM, segmentOverride)))
			{
				AAMPLOG_ERR("%s: Error on sending segment override event", __FUNCTION__);
			}
		}
	}
