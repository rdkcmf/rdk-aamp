...
static void AAMPGstPlayer_OnGstBufferUnderflowCb(GstElement* object, guint arg0, gpointer arg1,
        AAMPGstPlayer * _this)
{
	if (!gpGlobalConfig->bDisableUnderflow) // temp hack to test the video freeze issue.
	{
	...
	{
		logprintf("## %s() : [WARN] Ignored underflow config enabled, underflow from %s ##", __FUNCTION__, GST_ELEMENT_NAME(object));
	}
}
...
#define MAX_BYTES_TO_SEND (128*1024)
	...
	else
	{
		...
		maxBytes = len0; // workaround needed for Broadcom
#else
