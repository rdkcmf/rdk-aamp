...
static void AAMPGstPlayer_SendPendingEvents(PrivateInstanceAAMP *aamp, AAMPGstPlayerPriv *privateContext, MediaType mediaType, GstClockTime pts)
{
...
#endif
	logprintf("Sending segment event for mediaType[%d]. start %" G_GUINT64_FORMAT " stop %" G_GUINT64_FORMAT" rate %d applied_rate %d\n", mediaType, segment.start, segment.stop, segment.rate, segment.applied_rate);
	...
	if (stream->format == FORMAT_ISO_BMFF)
	{
		...
		enableOverride = TRUE;
#else
		enableOverride = (privateContext->rate != 1);
#endif
...
#ifdef TRACE_VID_PTS
	if (mediaType == eMEDIATYPE_VIDEO && privateContext->rate != 1)
	{
