...
GlobalConfigAAMP::GlobalConfigAAMP():
	...
	mAsyncTuneConfig(eTrueState), mWesterosSinkConfig(eUndefinedState), mPropagateUriParameters(eTrueState), aampRemovePersistent(0), preplaybuffercount(DEFAULT_PREBUFFER_COUNT),
	...
	initFragmentRetryCount(-1), unknownValues(), useMatchingBaseUrl(eUndefinedState), bEnableSubtec(true), bWebVttNative(true), bUseBasePts(false),
