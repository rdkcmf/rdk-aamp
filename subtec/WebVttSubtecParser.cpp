...
WebVTTSubtecParser::WebVTTSubtecParser(PrivateInstanceAAMP *aamp, SubtitleMimeType type) : SubtitleParser(aamp, type), m_channel(nullptr)
{
	...
	{
		AAMPLOG_INFO(mAamp->mConfig, "Init failed to connect to subtitle renderer - subtitle parsing disabled\n");
		throw std::runtime_error("PacketSender init failed");
	}

	m_channel = make_unique<WebVttChannel>();
	AAMPLOG_INFO(mAamp->mConfig, "Sending RESET ALL");
...
{
	AAMPLOG_INFO(mAamp->mConfig, "positionMs %lld", positionMs);
...
{
	AAMPLOG_INFO(mAamp->mConfig, "startPos %f start_ms %lld", startPos, start_ms_);
