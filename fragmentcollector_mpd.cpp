 ...
 */
bool PrivateStreamAbstractionMPD::ProcessEventStream(uint64_t startMS, IPeriod * period, bool reportBulkMeta)
{
	bool ret = false;
	const std::string &prdId = period->GetId();
	if(!prdId.empty())
 ...
 */
void PrivateStreamAbstractionMPD::AdvanceTrack(int trackIdx, bool trickPlay, double delta, bool *waitForFreeFrag, bool *exitFetchLoop, bool *bCacheFullState)
{
					...
					for (int trackIdx = (mNumberOfTracks - 1); trackIdx >= 0; trackIdx--)
					{
						if (aamp->mDashParallelFragDownload && trackIdx > 0) // (trackIdx > 0) indicates video/iframe/audio-only has to be downloaded in sync mode from this FetcherLoop().
						{
							...
							parallelDownload[trackIdx] = new std::thread(
												&PrivateStreamAbstractionMPD::AdvanceTrack,
												this,
												trackIdx,
												trickPlay,
												delta,
 ...
 */
bool PrivateStreamAbstractionMPD::isAdbreakStart(IPeriod *period, uint64_t &startMS, std::vector<std::pair<std::string, uint32_t>> &scte35Vec)
{
