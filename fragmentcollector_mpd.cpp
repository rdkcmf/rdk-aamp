...
static double ComputeFragmentDuration( uint32_t duration, uint32_t timeScale )
{
	...
	AAMPLOG_WARN_GP( "bad fragment duration");
	return newduration;
}
...
public:
	...
	)
	{
		...
		{
			AAMPLOG_INFO(aamp->mConfig, "Bitrate changed from %u to %ld", fragmentDescriptor.Bandwidth, bitrate);
		...
		else if (!ret)
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "LoadFragment failed");

				if (initSegment)
				{
					AAMPLOG_WARN(aamp->mConfig, "Init fragment fetch failed. fragmentUrl %s", fragmentUrl.c_str());
				}
				...
				if (MAX_SEG_DOWNLOAD_FAIL_COUNT <= segDLFailCount)
				{
					if(!playingAd)	//If playingAd, we are invalidating the current Ad in onAdEvent().
					{
						if (!initSegment)
						{
							AAMPLOG_ERR(aamp->mConfig, "Not able to download fragments; reached failure threshold sending tune failed event");
						...
						else
						{
							...
							AAMPLOG_ERR(aamp->mConfig, "Not able to download init fragments; reached failure threshold sending tune failed event");
				...
				else if ((eTRACK_VIDEO == type) && !(context->CheckForRampDownLimitReached()))
				{
					...
					if (context->CheckForRampDownProfile(http_code))
					{
						...
						AAMPLOG_WARN(aamp->mConfig,  "StreamAbstractionAAMP_MPD: Error while fetching fragment:%s, failedCount:%d. decrementing profile",
					...
					else
					{
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Already at the lowest profile, skipping segment");
				...
				{
					AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Error on fetching %s fragment. failedCount:%d",
							name, segDLFailCount);
		...
		else
		{
			...
			if (discontinuity)
			{
				AAMPLOG_WARN(aamp->mConfig, "Discontinuous fragment");
			}
	...
	void ABRProfileChanged(void)
	{
		...
		if (!((adaptationSetIdx == adaptIdxFromProfile) && (representationIndex == reprIdxFromProfile)))
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: - ABR %dx%d[%d] -> %dx%d[%d]",
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "representation is null");  //CID:83962 - Null Returns
			}
		}
		else
		{
	...
	, mMaxTracks(0)
{
	...
	switch (aamp->GetPreferredDRM())
	{
		case eDRM_WideVine:
		{
			AAMPLOG_INFO(aamp->mConfig, "DRM Selected: WideVine");
		...
		case eDRM_ClearKey:
		{
			AAMPLOG_INFO(aamp->mConfig, "DRM Selected: ClearKey");
		...
		default:
		{
			AAMPLOG_INFO(aamp->mConfig, "DRM Selected: PlayReady");
			mDrmPrefs[PLAYREADY_UUID] = highestPref+1;
		}
			break;
	}

	AAMPLOG_INFO(aamp->mConfig, "DRM prefs");
	for (auto const& pair: mDrmPrefs) {
		AAMPLOG_INFO(aamp->mConfig, "{ %s, %d }", pair.first.c_str(), pair.second);
	}
...
static bool IsAtmosAudio(const IMPDElement *nodePtr)
{
	...
	if (!nodePtr){
		AAMPLOG_ERR_GP("API Failed due to Invalid Arguments");
	}else{
				...
				if (childNode->HasAttribute("schemeIdUri")){
					...
					if (schemeIdUri == SCHEME_ID_URI_EC3_EXT_CODEC ){
						if (childNode->HasAttribute("value")) {
							...
							AAMPLOG_INFO_GP("Recieved %s tag property value as %s ",
					...
					{
						AAMPLOG_WARN_GP("schemeIdUri is not equals to  SCHEME_ID_URI_EC3_EXT_CODEC ");  //CID:84346 - Null Returns
					}
				}
			}
		}
	...
	if(adaptationSet != NULL)
	{
		...
		for (int representationIndex = 0; representationIndex < representation.size(); representationIndex++)
		{
			...
			else if (codecValue == "ec-3")
			{
				...
				*/
				if (IsAtmosAudio(rep)){
					AAMPLOG_INFO_GP("Setting audio codec as eAUDIO_ATMOS as per ETSI TS 103 420");
					audioType = eAUDIO_ATMOS;
				}
			}
			...
			*/
				...
				)
			{
				...
				AAMPLOG_INFO_GP("StreamAbstractionAAMP_MPD: SelectedRepIndex : %d ,selectedCodecType : %d, selectedRepBandwidth: %d", selectedRepIdx, selectedCodecType, selectedRepBandwidth);
			}
		}
	}
	else
	{
		AAMPLOG_WARN_GP("adaptationSet  is null");  //CID:85233 - Null Returns
	}
...
static bool IsContentType(IAdaptationSet *adaptationSet, MediaType mediaType )
{
	...
	if(name != NULL)
	{
		...
		{
			AAMPLOG_WARN_GP("excluding muxed content");
		}
		else
		{
	...
	{
		AAMPLOG_WARN_GP("name is null");  //CID:86093 - Null Returns
	}
	return false;
}
...
static bool ParseSegmentIndexBox( const char *start, size_t size, int segmentIndex, unsigned int *referenced_size, float *referenced_duration )
{
	...
	if (len != size) {
		AAMPLOG_WARN_GP("Wrong size in ParseSegmentIndexBox %d found, %zu expected", len, size);
		return false;
	}
	unsigned int type = Read32(f);
	if (type != 'sidx') {
		AAMPLOG_WARN_GP("Wrong type in ParseSegmentIndexBox %c%c%c%c found, %zu expected",
...
static int replace(std::string& str, const std::string& from, uint64_t toNumber )
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			{
				AAMPLOG_WARN_GP("next is not found ");  //CID:81252 - checked return
				break;
			}
		}
		if (done) break;
...
static int replace(std::string& str, const std::string& from, const std::string& toString )
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			{
				AAMPLOG_WARN_GP("Error at  next");  //CID:81346 - checked return
				break;
			}
		}
...
void StreamAbstractionAAMP_MPD::GetFragmentUrl( std::string& fragmentUrl, const FragmentDescriptor *fragmentDescriptor, std::string media)
{
	...
	else if (!constructedUri.empty())
	{
		if(ISCONFIGSET(eAAMPConfig_DASHIgnoreBaseURLIfSlash))
		{
			if (constructedUri == "/")
			{
				AAMPLOG_WARN(aamp->mConfig, "ignoring baseurl /");
				constructedUri.clear();
			}
		}
	...
	{
		AAMPLOG_TRACE(aamp->mConfig, "BaseURL not available");
	}
...
{
	AAMPLOG_WARN_GP("indexedTileInfo size=%d",indexedTileInfo.size());
...
{ // given url, synchronously download and transmit associated fragment
	...
	if(isInitializationSegment)
	{
		...
		{
			AAMPLOG_TRACE(aamp->mConfig, "We have pushed the same initailization segment for %s skipping", getMediaTypeName(MediaType(pMediaStreamContext->type)));
			return retval;
		}
		else
		{
	...
	if(rate > AAMP_NORMAL_PLAY_RATE)
	{
		...
		AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD:rate %f pMediaStreamContext->fragmentTime %f updated position %f",
...
//	AAMPLOG_WARN(aamp->mConfig, " [%s] mFirstFragPTS %f  position %f -> %f ", pMediaStreamContext->name, mFirstFragPTS[pMediaStreamContext->mediaType], position, mFirstFragPTS[pMediaStreamContext->mediaType]+position);
	...
	if (!fragmentCached)
	{
		if(!fragmentSaved)
		{
		AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: failed. fragmentUrl %s fragmentTime %f", fragmentUrl.c_str(), pMediaStreamContext->fragmentTime);
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: [CDAI] Ad fragment not available. Playback failed.");
...
bool StreamAbstractionAAMP_MPD::PushNextFragment( class MediaStreamContext *pMediaStreamContext, unsigned int curlInstance)
{
	...
	AAMPLOG_WARN(aamp->mConfig, " Type[%d] timeLineIndex %d fragmentRepeatCount %u PeriodDuration:%f mCurrentPeriodIdx:%d mPeriodStartTime %f",pMediaStreamContext->type,
	...
	if( segmentTemplates.HasSegmentTemplate() )
	{
		...
		if (segmentTimeline)
		{
			...
			{
#ifdef DEBUG_TIMELINE
				AAMPLOG_WARN(aamp->mConfig, " Type[%d] timelineCnt=%d timeLineIndex:%d FDTime=%f L=%" PRIu64 " [fragmentTime = %f,  mLiveEndPosition = %f]",
				...
				{
					AAMPLOG_INFO(aamp->mConfig, "Type[%d] EOS. timeLineIndex[%d] size [%lu]",pMediaStreamContext->type, pMediaStreamContext->timeLineIndex, timelines.size());
				...
				else
				{
						...
						&& pMediaStreamContext->fragmentDescriptor.Time == 0)
					{
						...
						if(startTime != presentationTimeOffset)
						{
							...
							if(startTime == 0)
							{
								AAMPLOG_INFO(aamp->mConfig, "Type[%d] Setting start time with PTSOffset:%" PRIu64 "",pMediaStreamContext->type,presentationTimeOffset);
								startTime = presentationTimeOffset;
							}
							else
							{
								...
								if(index != timelines.size())
								{
									...
									AAMPLOG_INFO(aamp->mConfig, "Type[%d] skipping fragments[%d] to Index:%d FNum=%d Repeat:%d", pMediaStreamContext->type,offsetNumber,index,pMediaStreamContext->fragmentDescriptor.Number,pMediaStreamContext->fragmentRepeatCount);
								}
							}
						}
					...
					else if (pMediaStreamContext->fragmentRepeatCount == 0)
					{
						...
						if(mIsLiveStream)
						{
							...
							if(0 == pMediaStreamContext->fragmentDescriptor.Time)
							{
								...
								*/
								if(index == timelines.size())
								{
									AAMPLOG_WARN(aamp->mConfig, " Type[%d] Boundary Condition !!! Index(%d) reached Max.Start=%" PRIu64 " Last=%" PRIu64 " ",
...
#ifdef DEBUG_TIMELINE
								AAMPLOG_WARN(aamp->mConfig, " Type[%d] t=%" PRIu64 " L=%" PRIu64 " d=%d r=%d Index=%d Num=%" PRIu64 " FTime=%f", pMediaStreamContext->type,
...
#ifdef DEBUG_TIMELINE
								AAMPLOG_WARN(aamp->mConfig, " Type[%d] t=%" PRIu64 " L=%" PRIu64 " d=%d r=%d fragRep=%d Index=%d Num=%" PRIu64 " FTime=%f", pMediaStreamContext->type,
						...
						{
							AAMPLOG_INFO(aamp->mConfig, "Type[%d] update startTime to %" PRIu64 ,pMediaStreamContext->type, startTime);
						}
						pMediaStreamContext->fragmentDescriptor.Time = startTime;
#ifdef DEBUG_TIMELINE
						AAMPLOG_WARN(aamp->mConfig, " Type[%d] Setting startTime to %" PRIu64 ,pMediaStreamContext->type, startTime);
#endif
					}// if fragRepeat == 0
...
#ifdef DEBUG_TIMELINE
					AAMPLOG_WARN(aamp->mConfig, " Type[%d] FDt=%f L=%" PRIu64 " d=%d r=%d fragrep=%d x=%d num=%lld",
...
#ifdef DEBUG_TIMELINE
						AAMPLOG_WARN(aamp->mConfig, " Type[%d] presenting FDt%f Number(%lld) Last=%" PRIu64 " Duration(%d) FTime(%f) endTime:%f",
					...
					{
#ifdef DEBUG_TIMELINE
						AAMPLOG_WARN(aamp->mConfig, " Type[%d] presenting %f" ,pMediaStreamContext->type,pMediaStreamContext->fragmentDescriptor.Time);
#endif
						pMediaStreamContext->lastSegmentTime = pMediaStreamContext->fragmentDescriptor.Time;
						pMediaStreamContext->lastSegmentDuration = pMediaStreamContext->fragmentDescriptor.Time + duration;
						double fragmentDuration = ComputeFragmentDuration(duration,timeScale);
							...
							((pMediaStreamContext->lastSegmentTime - pMediaStreamContext->fragmentDescriptor.Time) > TIMELINE_START_RESET_DIFF))
					{
						...
						AAMPLOG_WARN(aamp->mConfig, " Calling ScheduleRetune to handle start-time reset lastSegmentTime=%" PRIu64 " start-time=%f" , pMediaStreamContext->lastSegmentTime, pMediaStreamContext->fragmentDescriptor.Time);
						aamp->ScheduleRetune(eDASH_ERROR_STARTTIME_RESET, pMediaStreamContext->mediaType);
					}
					else
					{
#ifdef DEBUG_TIMELINE
						AAMPLOG_WARN(aamp->mConfig, " Type[%d] Before skipping. fragmentDescriptor.Time %f lastSegmentTime %" PRIu64 " Index=%d fragRep=%d,repMax=%d Number=%lld",pMediaStreamContext->type,
...
#ifdef DEBUG_TIMELINE
						AAMPLOG_WARN(aamp->mConfig, " Type[%d] After skipping. fragmentDescriptor.Time %f lastSegmentTime %" PRIu64 " Index=%d Number=%lld",pMediaStreamContext->type,
...
#ifdef DEBUG_TIMELINE
							AAMPLOG_WARN(aamp->mConfig, " Type[%d] After Incr. fragmentDescriptor.Time %f lastSegmentTime %" PRIu64 " Index=%d fragRep=%d,repMax=%d Number=%lld",pMediaStreamContext->type,
							pMediaStreamContext->fragmentDescriptor.Time, pMediaStreamContext->lastSegmentTime,pMediaStreamContext->timeLineIndex,
							pMediaStreamContext->fragmentRepeatCount , repeatCount,pMediaStreamContext->fragmentDescriptor.Number);
#endif
					}
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "timelines is null");  //CID:81702 ,82851 - Null Returns
			}
		}
		else
		{
#ifdef DEBUG_TIMELINE
			AAMPLOG_WARN(aamp->mConfig, " segmentTimeline not available");
#endif
			...
			if (0 == pMediaStreamContext->lastSegmentNumber)
			{
				if (mIsLiveStream)
				{
					...
					AAMPLOG_INFO(aamp->mConfig, "Printing fragmentDescriptor.Number %" PRIu64 " Time=%f  ", pMediaStreamContext->lastSegmentNumber, pMediaStreamContext->fragmentDescriptor.Time);
				}
				else
				{
					if (rate < 0)
			...
			{
				AAMPLOG_INFO(aamp->mConfig, "EOS. fragmentDescriptor.Time=%f mPeriodEndTime=%lu mPeriodStartTime %lu  currentTimeSeconds %f FTime=%f", pMediaStreamContext->fragmentDescriptor.Time, mPeriodEndTime, mPeriodStartTime, currentTimeSeconds, pMediaStreamContext->fragmentTime);
			...
			else if(mIsLiveStream && (pMediaStreamContext->fragmentDescriptor.Time + fragmentDuration) >= (currentTimeSeconds-mPresentationOffsetDelay))
			{
				...
				AAMPLOG_INFO(aamp->mConfig, "Next fragment Not Available yet: fragmentDescriptor.Time %f currentTimeSeconds %f sleepTime %d ", pMediaStreamContext->fragmentDescriptor.Time, currentTimeSeconds, sleepTime);
		...
		{ // single-segment
			...
			{ // lazily load index
				...
				if (pMediaStreamContext->fragmentIndex != 0)
				{
					...
					AAMPLOG_INFO(aamp->mConfig, "current fragmentIndex = %d", pMediaStreamContext->fragmentIndex);
			...
			if (pMediaStreamContext->index_ptr)
			{
				...
				if (ParseSegmentIndexBox(pMediaStreamContext->index_ptr, pMediaStreamContext->index_len, pMediaStreamContext->fragmentIndex++, &referenced_size, &fragmentDuration))
				{
					...
					AAMPLOG_INFO(aamp->mConfig, "%s [%s]",getMediaTypeName(pMediaStreamContext->mediaType), range);
				...
				else if(!segmentURLs.empty())
				{
					...
					if(segmentURL != NULL)
					{
						...
						if(rawAttributes.find("customlist") == rawAttributes.end()) //"CheckForFogSegmentList")
						{
							...
							AAMPLOG_INFO(aamp->mConfig, "%s [%s]", getMediaTypeName(pMediaStreamContext->mediaType), segmentURL->GetMediaRange().c_str());
						...
						else //We are procesing the custom segment list provided by Fog for DASH TSB
						{
							...
							{
								AAMPLOG_WARN(aamp->mConfig, " START-TIME RESET in TSB period, lastSegmentTime=%" PRIu64 " start-time=%lld duration=%lld", pMediaStreamContext->lastSegmentTime, startTime, duration);
							...
							else
							{
								...
								*/
								...
								AAMPLOG_TRACE(aamp->mConfig, "PushNextFragment Exit : startTime %lld lastSegmentTime %lu index = %d", startTime, pMediaStreamContext->lastSegmentTime, pMediaStreamContext->fragmentIndex);
							}
						}
						if(rate > 0)
						{
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "segmentURL    is null");  //CID:82493 ,86180 - Null Returns
					}
				}
				else
				{
					AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: SegmentUrl is empty");
				}
			}
			else
			{
				AAMPLOG_ERR_GP("not-yet-supported mpd format");
			}
		}
	}
	return retval;
...
void StreamAbstractionAAMP_MPD::SkipToEnd( MediaStreamContext *pMediaStreamContext)
{
	...
	if( segmentTemplates.HasSegmentTemplate() )
	{
		...
		if (segmentTimeline)
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "timelines is null");  //CID:82016,84031 - Null Returns
			}
		}
		else
		{
...
double StreamAbstractionAAMP_MPD::SkipFragments( MediaStreamContext *pMediaStreamContext, double skipTime, bool updateFirstPTS)
{
	...
	if( segmentTemplates.HasSegmentTemplate() )
	{
		 AAMPLOG_INFO(aamp->mConfig, "%s:%d Enter : Type[%d] timeLineIndex %d fragmentRepeatCount %d fragmentTime %f skipTime %f segNumber %lu", __FUNCTION__, __LINE__,pMediaStreamContext->type,
...
//					AAMPLOG_TRACE(aamp->mConfig, "[%s] firstPTS %f nextPTS %f duration %f skipTime %f", pMediaStreamContext->name, firstPTS, nextPTS, fragmentDuration, skipTime);
...
//							AAMPLOG_INFO(aamp->mConfig, "newSkiptime %f, skipTime %f  mFirstFragPTS[vid] %f  firstPTS %f  mFirstFragPTS[vid]-firstPTS %f mVideoPosRemainder %f  fragmentDuration/4.0 %f", newSkipTime, skipTime, mFirstFragPTS[eTRACK_VIDEO], firstPTS, mFirstFragPTS[eTRACK_VIDEO]-firstPTS, mVideoPosRemainder,  fragmentDuration/4.0);
					...
					{ // last iteration
						AAMPLOG_INFO(aamp->mConfig, "[%s] firstPTS %f, nextPTS %f  skipTime %f  fragmentDuration %f ", pMediaStreamContext->name, firstPTS, nextPTS, skipTime, fragmentDuration);
						if (updateFirstPTS)
						{
							/*Keep the lower PTS */
							if ( ((mFirstPTS == 0) || (firstPTS < mFirstPTS)) && (pMediaStreamContext->type == eTRACK_VIDEO))
							{
								AAMPLOG_INFO(aamp->mConfig, "[%s] mFirstPTS %f -> %f ", pMediaStreamContext->name, mFirstPTS, firstPTS);
								mFirstPTS = firstPTS;
								...
								AAMPLOG_INFO(aamp->mConfig, "[%s] mFirstPTS %f  mVideoPosRemainder %f", pMediaStreamContext->name, mFirstPTS, mVideoPosRemainder);
							}
						}
						skipTime = 0;
						if (pMediaStreamContext->type == eTRACK_AUDIO){
//							AAMPLOG_TRACE(aamp->mConfig, "audio/video PTS offset %f  audio %f video %f", firstPTS-mFirstPTS, firstPTS, mFirstPTS);
							if (abs(firstPTS - mFirstPTS)> 1.00){
								AAMPLOG_WARN(aamp->mConfig, "audio/video PTS offset Large %f  audio %f video %f",  firstPTS-mFirstPTS, firstPTS, mFirstPTS);
							}
						}
						break;
					}
			...
			else
			{
				...
				{
					AAMPLOG_INFO(aamp->mConfig, "Type[%d] EOS. fragmentDescriptor.Time=%f",pMediaStreamContext->type, pMediaStreamContext->fragmentDescriptor.Time);
					pMediaStreamContext->eos = true;
					break;
				}
				else
			...
			if( skipTime==0 ) AAMPLOG_WARN(aamp->mConfig,  "XIONE-941" );
		}while(true); // was while(skipTime != 0);

		AAMPLOG_INFO(aamp->mConfig, "Exit :Type[%d] timeLineIndex %d fragmentRepeatCount %d fragmentDescriptor.Number %" PRIu64 " fragmentTime %f FTime:%f",pMediaStreamContext->type,
	...
	else
	{
		...
		if (segmentList)
		{
			AAMPLOG_INFO(aamp->mConfig, "Enter : fragmentIndex %d skipTime %f",
				...
				else if(pMediaStreamContext->type == eTRACK_AUDIO)
				{
					...
					if(videoContext != NULL)
					{
						...
						if(!vidSegmentURLs.empty())
						{
							...
							AAMPLOG_WARN(aamp->mConfig, "Printing diff value for adjusting %lld",diff);
							if(diff > 0)
							{
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Video SegmentUrl is empty");
						}
					}
					else
					{
						AAMPLOG_WARN(aamp->mConfig, "videoContext  is null");  //CID:82361 - Null Returns
					}
				}
			...
			AAMPLOG_INFO(aamp->mConfig, "Exit : fragmentIndex %d segmentDuration %f",
...
static void AddAttributesToNode(xmlTextReaderPtr *reader, Node *node)
{
	if (xmlTextReaderHasAttributes(*reader))
	{
		while (xmlTextReaderMoveToNextAttribute(*reader))
		{
			...
			{
				AAMPLOG_WARN_GP("key   is null");  //CID:85916 - Null Returns
			}
		}
	}
}
...
static uint64_t ParseISO8601Duration(const char *ptr)
{
	...
	{
		AAMPLOG_WARN_GP(" - Invalid input %s", ptr);
	}
...
std::shared_ptr<AampDrmHelper> StreamAbstractionAAMP_MPD::CreateDrmHelper(IAdaptationSet * adaptationSet,MediaType mediaType)
{
	...
	AAMPLOG_TRACE(aamp->mConfig, "[HHH] contentProt.size= %d", contentProt.size());
	for (unsigned iContentProt = 0; iContentProt < contentProt.size(); iContentProt++)
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "(%s) got schemeID empty at ContentProtection node-%d", getMediaTypeName(mediaType), iContentProt);
			continue;
		}
		...
		if (!node.empty())
		{
			...
			if (tagName.find("pssh") != std::string::npos)
			{
				...
				if (0 == dataLength)
				{
					AAMPLOG_WARN(aamp->mConfig, "base64_Decode of pssh resulted in 0 length");
					if (data)
					{
						free(data);
						data = NULL;
		...
		if (aamp->mIsWVKIDWorkaround && drmInfo.systemUUID == CLEARKEY_UUID ){
			...
			AAMPLOG_INFO(aamp->mConfig, "WideVine KeyID workaround present, processing KeyID from clear key to WV Data");
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "(%s) Failed to locate DRM helper for UUID %s", getMediaTypeName(mediaType), drmInfo.systemUUID.c_str());
			...
			if(aamp->isPreferredDRMConfigured && (GetPreferredDrmUUID() == drmInfo.systemUUID) && !aamp->mIsWVKIDWorkaround){
				AAMPLOG_ERR(aamp->mConfig, "(%s) Preffered DRM Failed to locate with UUID %s", getMediaTypeName(mediaType), drmInfo.systemUUID.c_str());
				break;
			}
		}
		else if (data && dataLength)
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "(%s) Failed to Parse PSSH from the DRM InitData", getMediaTypeName(mediaType));
			}
			else
			{
				if (forceSelectDRM){
					AAMPLOG_INFO(aamp->mConfig, "(%s) If Widevine DRM Selected due to Widevine KeyID workaround",
				...
				{
					AAMPLOG_WARN(aamp->mConfig, " (%s) Created DRM helper for UUID %s and best to use", getMediaTypeName(mediaType), drmInfo.systemUUID.c_str());
					drmHelper = tmpDrmHelper;
				}
			}
		}
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "(%s) No PSSH data available from the stream for UUID %s", getMediaTypeName(mediaType), drmInfo.systemUUID.c_str());
			/** Preferred DRM configured and it is failed then exit here */
			if(aamp->isPreferredDRMConfigured && (GetPreferredDrmUUID() == drmInfo.systemUUID)&& !aamp->mIsWVKIDWorkaround){
				AAMPLOG_ERR(aamp->mConfig, "(%s) No PSSH data available for Preffered DRM with UUID  %s", getMediaTypeName(mediaType), drmInfo.systemUUID.c_str());
				break;
			}
		}
	...
	if((drmHelper) && (!drmHelper->compare(mLastDrmHelper)))
	{
		...
		if(sessionParams != NULL)
		{
			...
			if(drmSessionThreadStarted) //In the case of license rotation
			{
				...
				if (rc != 0)
				{
					AAMPLOG_ERR(aamp->mConfig, "(%s) pthread_join returned %d for createDRMSession Thread", getMediaTypeName(mediaType), rc);
				}
			...
			*/
			...
			{
				AAMPLOG_INFO(aamp->mConfig, "Thread created");
			...
			{
				AAMPLOG_ERR(aamp->mConfig, "(%s) pthread_create failed for CreateDRMSession : error code %d, %s", getMediaTypeName(mediaType), errno, strerror(errno));
			}
		}
		else
		{
			AAMPLOG_WARN(aamp->mConfig, "sessionParams  is null");  //CID:85612 - Null Returns
		}
	}
	else
	{
		AAMPLOG_WARN(aamp->mConfig, "(%s) Skipping creation of session for duplicate helper", getMediaTypeName(mediaType));
	}
}
...
void StreamAbstractionAAMP_MPD::ProcessContentProtection(IAdaptationSet * adaptationSet, MediaType mediaType, std::shared_ptr<AampDrmHelper> drmHelper)
{
	...
	{
		AAMPLOG_ERR(aamp->mConfig, "(%s) Failed to create DRM helper", getMediaTypeName(mediaType));
	}
	else
	{
		AAMPLOG_WARN(aamp->mConfig, "(%s) Skipping creation of session for duplicate helper", getMediaTypeName(mediaType));
...
void StreamAbstractionAAMP_MPD::ProcessContentProtection(IAdaptationSet * adaptationSet,MediaType mediaType, std::shared_ptr<AampDrmHelper> drmHelper)
{
	AAMPLOG_WARN(aamp->mConfig, "MPD DRM not enabled");
}
#endif
	...
	if( adaptationSets.size() > 0 )
	{
		...
		if (segmentTemplates.HasSegmentTemplate())
		{
			...
			if (segmentTimeline)
			{
				uint32_t timeScale = segmentTemplates.GetTimescale();
				uint64_t presentationTimeOffset = segmentTemplates.GetPresentationTimeOffset();
				AAMPLOG_WARN_GP("tscale: %" PRIu64 " offset : %" PRIu64 "", timeScale, presentationTimeOffset);
				std::vector<ITimeline *>&timelines = segmentTimeline->GetTimelines();
				ITimeline *timeline = timelines.at(0);
				if(timeline != NULL)
				{
					uint64_t timelineStart = timeline->GetStartTime();
					AAMPLOG_WARN_GP("timeline start : %" PRIu64 "", timelineStart);
	...
	if(mpd != NULL)
	{
		...
		if(periodIndex < periodCnt)
		{
			...
			if(!startTimeStr.empty())
			{
				...
				AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: MPD periodIndex %d AvailStartTime %f periodStart %f %s", periodIndex, mAvailabilityStartTime, periodStart,startTimeStr.c_str());
			}
			else
			{
				...
				AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: MPD periodIndex %d periodStart %f", periodIndex, periodStart);
			}
		}
	}
	else
	{
		AAMPLOG_WARN(aamp->mConfig, "mpd is null");  //CID:83436 Null Returns
	}
	return periodStart;
}
	...
	if(mpd != NULL)
	{
		...
		if(periodIndex < periodCnt)
		{
			...
			if(!durationStr.empty())
			{
				...
				AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: MPD periodIndex:%d periodDuration %f", periodIndex, periodDuration);
			}
			else
			{
...
double StreamAbstractionAAMP_MPD::GetPeriodEndTime(IMPD *mpd, int periodIndex, uint64_t mpdRefreshTime)
{
	...
	{
		AAMPLOG_WARN(aamp->mConfig, "mpd is null");  //CID:80459 , 80529- Null returns
	}
	if(period != NULL)
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "availabilityStartTime required to calculate period duration not present in MPD");
		}
		else if(0 == periodDurationMs)
		{
			AAMPLOG_WARN(aamp->mConfig, "Could not get valid period duration to calculate end time");
		}
		else
		{
			if(startTimeStr.empty())
			{
				AAMPLOG_WARN(aamp->mConfig, "Period startTime required to calculate period duration not present in MPD");
			}
			else
			{
				periodStartMs = ParseISO8601Duration(startTimeStr.c_str());
		...
		AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: MPD periodIndex:%d periodEndTime %f", periodIndex, periodEndTime);
	}
	else
	{
		AAMPLOG_WARN(aamp->mConfig, "period is null");  //CID:85519- Null returns
	}
...
uint64_t aamp_GetPeriodDuration(dash::mpd::IMPD *mpd, int periodIndex, uint64_t mpdDownloadTime)
{
		...
		if(0 == durationMs && mpd->GetType() == "static" && numPeriods == 1)
	{
		...
		{
			AAMPLOG_WARN_GP("mediaPresentationDuration missing in period %s", period->GetId().c_str());
		}
	}
	if(0 == durationMs)
	{
		...
		if (adaptationSets.size() > 0)
		{
				...
				if( segmentTemplates.HasSegmentTemplate() )
				{
					...
					if (segmentTimeline)
					{
						...
						while (timeLineIndex < timelines.size())
						{
							...
							AAMPLOG_TRACE_GP("timeLineIndex[%d] size [%lu] updated durationMs[%" PRIu64 "]", timeLineIndex, timelines.size(), durationMs);
							timeLineIndex++;
						}
					}
					else
					{
						...
						if(!periodStartStr.empty())
						{
							...
							else if(periodIndex == (periods.size() - 1))
							{
								...
								{
									AAMPLOG_WARN_GP("mpdDownloadTime required to calculate period duration not provided");
								}
								else if(minimumUpdatePeriodStr.empty())
								{
									AAMPLOG_WARN_GP("minimumUpdatePeriod required to calculate period duration not present in MPD");
								}
								else if(availabilityStartStr.empty())
								{
									AAMPLOG_WARN_GP("availabilityStartTime required to calculate period duration not present in MPD");
								}
								else
								{
									uint64_t periodStart = 0;
									...
									AAMPLOG_INFO_GP("periodStart %lu availabilityStartTime %lu minUpdatePeriod %lu mpdDownloadTime %lu", periodStart, availablilityStart, minUpdatePeriod, mpdDownloadTime);
									...
									if(durationMs <= 0)
									{
										AAMPLOG_WARN_GP("Invalid period duration periodStartTime %lu periodEndTime %lu durationMs %lu", periodStartTime, periodEndTime, durationMs);
										durationMs = 0;
									}
								}
							}
							...
							else
							{
								...
								if(!nextPeriodStartStr.empty())
								{
									...
									if(durationMs <= 0)
									{
										AAMPLOG_WARN_GP("Invalid period duration periodStartTime %lu nextPeriodStart %lu durationMs %lu", periodStart, nextPeriodStart, durationMs);
										durationMs = 0;
									}
								}
								else
								{
									AAMPLOG_WARN_GP("Next period startTime missing periodIndex %d", periodIndex);
								}
							}
						}
						else
						{
							AAMPLOG_WARN_GP("Start time and duration missing in period %s", period->GetId().c_str());
						}
					}
				}
				else
				{
					...
					if (representations.size() > 0)
					{
						...
						if (segmentList)
						{
							...
							{
								AAMPLOG_WARN_GP("segmentURLs  is null");  //CID:82729 - Null Returns
							}
						}
						else
						{
			...
			{
				AAMPLOG_WARN_GP("firstAdaptation is null");  //CID:84261 - Null Returns
			}
		}
	}
	return durationMs;
...
AAMPStatusType StreamAbstractionAAMP_MPD::Init(TuneType tuneType)
{
	...
	if (ret == eAAMPSTATUS_OK)
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "mpd is null");  //CID:81139 , 81645 ,82315.83556- Null Returns
		}
		if(!tempString.empty())
		{
			durationMs = ParseISO8601Duration( tempString.c_str() );
			mpdDurationAvailable = true;
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: - MPD duration str %s val %" PRIu64 " seconds", tempString.c_str(), durationMs/1000);
		}
		...
		if(mIsLiveStream)
		{
			if (aamp->mIsVSS)
			{
				...
				{
					AAMPLOG_INFO(aamp->mConfig, "Virtual stream ID :%s", vssVirtualStreamId.c_str());
			...
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: - AvailabilityStartTime=%f", mAvailabilityStartTime);
			...
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: MPD minupdateduration val %" PRIu64 " seconds mTSBDepth %f mPresentationOffsetDelay :%f StartTimeFirstPeriod: %lf offsetStartTime: %lf",  mMinUpdateDurationMs/1000, mTSBDepth, mPresentationOffsetDelay, mFirstPeriodStartTime, aamp->mProgressReportOffset);
		}
		...
		{//TODO -  test with streams having multiple periods.
			...
			if(!tempString.empty())
			{
				...
				if(!mpdDurationAvailable)
				{
					...
					AAMPLOG_WARN(aamp->mConfig, " - Updated duration %" PRIu64 " seconds", durationMs/1000);
				}
			}
			else if (mIsFogTSB)
			{
				...
				if (periodDurationMs != 0)
				{
					...
					if(prevPeriodEndMs > 0)
					{
						...
						if(periodGap > 0 ) // ohh we have GAP between last and current period
						{
							...
							AAMPLOG_WARN(aamp->mConfig, "GAP betwen period found :GAP:%f  mCurrentPeriodIdx %d currentPeriodStart %f offsetFromStart %f",
					...
					if (periodDurationSeconds <= offsetFromStart && iPeriod < (numPeriods - 1))
					{
						...
						AAMPLOG_WARN(aamp->mConfig, "Skipping period %d seekPosition %f periodEnd %f offsetFromStart %f", iPeriod, seekPosition, periodEnd, offsetFromStart);
						continue;
					}
					else
					{
		...
		if (0 == durationMs || (mpdDurationAvailable && mIsLiveStream && !mIsFogTSB))
		{
			...
			AAMPLOG_WARN(aamp->mConfig, " - Duration after GetDurationFromRepresentation %" PRIu64 " seconds", durationMs/1000);
		}
		...
		if(0 == durationMs)
		{
			...
			AAMPLOG_WARN(aamp->mConfig, " - Duration after adding up Period Duration %" PRIu64 " seconds", durationMs/1000);
		}
		...
		if (mIsLiveStream)
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: eTUNETYPE_SEEKTOLIVE");
				liveAdjust = true;
			...
			else if (((eTUNETYPE_SEEK == tuneType) || (eTUNETYPE_RETUNE == tuneType || eTUNETYPE_NEW_SEEK == tuneType)) && (rate > 0))
			{
				...
				{
					AAMPLOG_WARN(aamp->mConfig,  "StreamAbstractionAAMP_MPD: offSetFromStart[%f] seekWindowEnd[%f]",
			...
			if (liveAdjust)
			{
				...
				if(aamp->IsLiveAdjustRequired())
				{
					if(segmentTagsPresent)
					{
						...
						{
							AAMPLOG_INFO(aamp->mConfig, "Adjusting to live offset offsetFromStart %f, mCurrentPeriodIdx %d", offsetFromStart, mCurrentPeriodIdx);
							mCurrentPeriodIdx--;
							duration = (double)(aamp_GetPeriodDuration(mpd, mCurrentPeriodIdx, mLastPlaylistDownloadTimeMs)) / 1000;
					...
					AAMPLOG_WARN(aamp->mConfig, "duration %f durationMs %f mCurrentPeriodIdx %d currentPeriodStart %f offsetFromStart %f",
				...
				mIsAtLivePoint = true;
				AAMPLOG_WARN(aamp->mConfig,  "StreamAbstractionAAMP_MPD: - liveAdjust - Updated offSetFromStart[%f] duration [%f] currentPeriodStart[%f] MaxPeriodIdx[%d]",
		...
		else
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "Duration of VOD content is 0") ;
			...
			if(seekPosition > seekWindowEnd)
			{
				...
				AAMPLOG_WARN(aamp->mConfig, " seek target out of range, mark EOS. playTarget:%f End:%f. ",
					seekPosition, seekWindowEnd);
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "mCurrentPeriod  is null");  //CID:84770 - Null Return
		}
		...
		else if(mNumberOfTracks)
		{
			...
			if (eTUNED_EVENT_ON_PLAYLIST_INDEXED == aamp->GetTuneEventConfig(mIsLiveStream))
			{
				...
				{
					AAMPLOG_WARN(aamp->mConfig, "aamp: mpd - sent tune event after indexing playlist");
				}
			}
			...
			if(eAAMPSTATUS_OK != ret)
			{
				...
				{
					AAMPLOG_ERR(aamp->mConfig, "ERROR: No playable profiles found");
				}
				return ret;
			}
			...
			AAMPLOG_INFO(aamp->mConfig, "offsetFromStart(%f) seekPosition(%f) currentPeriodStart(%f)", offsetFromStart,seekPosition, currentPeriodStart);
			if (newTune )
			{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "No adaptation sets could be selected");
			retval = eAAMPSTATUS_MANIFEST_CONTENT_ERROR;
		}
	}
	else
	{
		AAMPLOG_ERR(aamp->mConfig, "StreamAbstractionAAMP_MPD: corrupt/invalid manifest");
	...
	if (ret == eAAMPSTATUS_OK)
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Pushing EncryptedHeaders");
			PushEncryptedHeaders();
		}
#endif

		AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: - fetch initialization fragments");
		FetchAndInjectInitialization();
	}
...
uint64_t aamp_GetDurationFromRepresentation(dash::mpd::IMPD *mpd)
{
	...
	for (unsigned iPeriod = 0; iPeriod < numPeriods; iPeriod++)
	{
		...
		{
			AAMPLOG_WARN_GP("mpd is null");  //CID:82158 - Null Returns
		}
		...
		if(period != NULL)
		{
			...
			if (adaptationSets.size() > 0)
			{
				...
				if (segmentTemplates.HasSegmentTemplate())
				{
					...
					{
						AAMPLOG_WARN_GP("media is null");  //CID:83185 - Null Returns
					}
				}
				else
				{
					...
					if (representations.size() > 0)
					{
						...
						if (segmentList)
						{
							...
							if(segmentURLs.empty())
							{
								AAMPLOG_WARN_GP("segmentURLs is null");  //CID:82113 - Null Returns
							}
							durationMs += ComputeFragmentDuration(segmentList->GetDuration(), segmentList->GetTimescale()) * 1000;
						}
						else
		...
		{
			AAMPLOG_WARN_GP("period is null");  //CID:81482 - Null Returns
		}
	}
	return durationMs;
}
...
AAMPStatusType StreamAbstractionAAMP_MPD::UpdateMPD(bool init)
{
	...
	{
		AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: manifest retrieved from cache");
	...
	if (!retrievedPlaylistFromCache)
	{
		...
		else if (aamp->DownloadsAreEnabled())
		{
			...
			if (this->mpd != NULL && (CURLE_OPERATION_TIMEDOUT == http_error || CURLE_COULDNT_CONNECT == http_error))
			{
				...
				AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Ignore curl timeout");
				ret = AAMPStatusType::eAAMPSTATUS_OK;
			}
			else
			{
				aamp->SendDownloadErrorEvent(AAMP_TUNE_MANIFEST_REQ_FAILED, http_error);
				AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: - manifest download failed");
				ret = AAMPStatusType::eAAMPSTATUS_MANIFEST_DOWNLOAD_ERROR;
			}
		}
		else // if downloads disabled
		{
			AAMPLOG_ERR(aamp->mConfig, "StreamAbstractionAAMP_MPD: - manifest download failed");
			ret = AAMPStatusType::eAAMPSTATUS_MANIFEST_DOWNLOAD_ERROR;
		}
	}
	else
	...
	if (gotManifest)
	{
		...
		{
			AAMPLOG_ERR(aamp->mConfig, " Error while processing MPD, GetMpdFromManfiest returned %d", ret);
	...
	{
		AAMPLOG_WARN(aamp->mConfig, "aamp: error on manifest fetch");
	}
	...
	if( ret == eAAMPSTATUS_MANIFEST_PARSE_ERROR || ret == eAAMPSTATUS_MANIFEST_CONTENT_ERROR)
	{
		if(NULL != manifest.ptr && !manifestUrl.empty())
		{
			...
			AAMPLOG_WARN(aamp->mConfig, "ERROR: Invalid Playlist URL: %s ret:%d", manifestUrl.c_str(),ret);
			AAMPLOG_WARN(aamp->mConfig, "ERROR: Invalid Playlist DATA: %s ", temp);
		}
...
void StreamAbstractionAAMP_MPD::FindTimedMetadata(MPD* mpd, Node* root, bool init, bool reportBulkMeta)
{
	...
	if(!subNodes.empty())
		...
		for (size_t i=0; i < subNodes.size(); i++) {
			...
			if(!node)
			{
				AAMPLOG_WARN(aamp->mConfig, "node is null");  //CID:80723 - Null Returns
			}
			...
			if (name == "Period") {
				...
				{
					AAMPLOG_WARN(aamp->mConfig, "mpd is null");  //CID:80941 - Null Returns
				}
				...
				if(period != NULL)
				{
					...
					for (size_t j=0; j < children.size(); j++) {
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "name is empty");  //CID:80526 - Null Returns
						}
					}
					if("" != prdId)
					{
				...
				for (size_t i=0; i < infoNodes.size(); i++) {
					...
					if (name == "ContentIdentifier" && (infoNodeType == "URI" || infoNodeType == "URN")) {
						if (infoNode->HasAttribute("value")) {
							...
							AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-CONTENT-IDENTIFIER:%s", 0.0f, content.c_str());
			...
			if (name == "SupplementalProperty" && node->HasAttribute("schemeIdUri")) {
				...
				if (schemeIdUri == schemeIdUriDai && node->HasAttribute("id")) {
					...
					if (ID == "identityADS" && node->HasAttribute("value")) {
						...
						AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-IDENTITY-ADS:%s", 0.0f, content.c_str());
					...
					if (ID == "messageRef" && node->HasAttribute("value")) {
						...
						AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-MESSAGE-REF:%s", 0.0f, content.c_str());
	...
	{
		AAMPLOG_WARN(aamp->mConfig, "SubNodes  is empty");  //CID:85449 - Null Returns
	}
}
...
void StreamAbstractionAAMP_MPD::ProcessPeriodSupplementalProperty(Node* node, std::string& AdID, uint64_t startMS, uint64_t durationMS, bool isInit, bool reportBulkMeta)
{
	if (node->HasAttribute("schemeIdUri")) {
			...
			if ((schemeIdUri == schemeIdUriDai) && node->HasAttribute("id")) {
				...
				if ((ID == "Tracking") && node->HasAttribute("value")) {
					...
					if (!value.empty()) {
						...
						AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-CUE:%s", start, content.c_str());
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "schemeIdUri  is empty");  //CID:83796 - Null Returns
		}
	}
}
...
void StreamAbstractionAAMP_MPD::ProcessPeriodAssetIdentifier(Node* node, uint64_t startMS, uint64_t durationMS, std::string& AssetID, std::string& ProviderID, bool isInit, bool reportBulkMeta)
{
	if (node->HasAttribute("schemeIdUri")) {
		...
		if ((schemeIdUri == "urn:cablelabs:md:xsd:content:3.0") && node->HasAttribute("value")) {
			...
			if (!value.empty()) {
				...
				if (pos != std::string::npos) {
					...
					std::string content = s.str();
					AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-ASSET-ID:%s", start, content.c_str());
		...
		else if ((schemeIdUri == "urn:scte:dash:asset-id:upid:2015"))
		{
			...
			std::string content = s.str();
			AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-SOURCE-STREAM:%s", start, content.c_str());
...
bool StreamAbstractionAAMP_MPD::ProcessEventStream(uint64_t startMS, IPeriod * period, bool reportBulkMeta)
{
	...
	if(!prdId.empty())
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "Found SCTE35 event for period %s ", prdId.c_str()); 
			for(std::pair<std::string, uint32_t> scte35 : scte35Vec)
			{
				...
				else
				{
					...
					if(reportBulkMeta)
					{
						AAMPLOG_INFO(aamp->mConfig, "Saving timedMetadata for VOD SCTE35 event for the period, %s", prdId.c_str()); 
						aamp->SaveTimedMetadata(startMS, "SCTE35", scte35.first.c_str(), scte35.first.size(), prdId.c_str(), scte35.second);
					}
					else
					{
						AAMPLOG_INFO(aamp->mConfig, "Sending timedMetadata for VOD SCTE35 event for the period, %s", prdId.c_str()); 
...
void StreamAbstractionAAMP_MPD::ProcessStreamRestrictionList(Node* node, const std::string& AdID, uint64_t startMS, bool isInit, bool reportBulkMeta)
{
	...
	{
		AAMPLOG_WARN(aamp->mConfig, "node is null");  //CID:84081 - Null Returns
	}
}
...
void StreamAbstractionAAMP_MPD::ProcessStreamRestriction(Node* node, const std::string& AdID, uint64_t startMS, bool isInit, bool reportBulkMeta)
{
	...
	for (size_t i=0; i < children.size(); i++) {
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "child is null");  //CID:84810 - Null Returns
		}
	}
}
...
void StreamAbstractionAAMP_MPD::ProcessTrickModeRestriction(Node* node, const std::string& AdID, uint64_t startMS, bool isInit, bool reportBulkMeta)
{
	...
	if (node->HasAttribute("trickMode")) {
		...
		if(!trickMode.length())
		{
			AAMPLOG_WARN(aamp->mConfig, "trickMode is null");  //CID:86122 - Null Returns
		}
	}
	...
	if (!trickMode.empty() && !text.empty()) {
		...
		std::string content = s.str();
		AAMPLOG_INFO(aamp->mConfig, "TimedMetadata: @%1.3f #EXT-X-TRICKMODE-RESTRICTION:%s", start, content.c_str());
...
static void * TrackDownloader(void *arg)
{
	...
	{
		AAMPLOG_ERR(fetchParms->context->aamp->mConfig,
...
static void * FragmentDownloader(void *arg)
{
	...
	{
		AAMPLOG_ERR(downloadParams->context->aamp->mConfig, ": aamp_pthread_setname failed");
	}
		...
		while (downloadParams->context->aamp->DownloadsAreEnabled() && !downloadParams->pMediaStreamContext->profileChanged)
		{
			...
			if(downloadParams->pMediaStreamContext->WaitForFreeFragmentAvailable(timeoutMs))
			{
				...
				if (downloadParams->pMediaStreamContext->eos)
				{
					...
					AAMPLOG_INFO(downloadParams->context->aamp->mConfig, "%s EOS - Exit fetch loop", downloadParams->pMediaStreamContext->name);
					break;
				}
			}
	...
	{
		AAMPLOG_WARN(downloadParams->context->aamp->mConfig, " NULL adaptationSet");
	}
	return NULL;
}
		...
		if(xml != NULL)
		{
			if (xml->GetName() == "EssentialProperty")
			{
				if (xml->HasAttribute("schemeIdUri"))
				{
					...
					{
						AAMPLOG_WARN_GP(" - skipping schemeUri %s", schemeUri.c_str());
					}
				}
			}
			else
			{
				AAMPLOG_TRACE_GP("- skipping name %s", xml->GetName().c_str());
			}
		}
		else
		{
			AAMPLOG_WARN_GP("xml is null");  //CID:81118 - Null Returns
		}
	}
	return false;
}
	...
	for (unsigned iPeriod = 0; iPeriod < numPeriods; iPeriod++)
	{
		...
		if(period != NULL)
		{
			...
			for (int iAdaptationSet = 0; iAdaptationSet < numAdaptationSets; iAdaptationSet++)
			{
				...
				{
					AAMPLOG_WARN(aamp->mConfig, "adaptationSet is null");  //CID:86317 - Null Returns
				}
			}
		}
		else
		{
			AAMPLOG_WARN(aamp->mConfig, "period is null");  //CID:83754 - Null Returns
		}
	}
...
{
	AAMPLOG_TRACE(aamp->mConfig, "Processing License request for pending KeyIDs: %d", mPendingKeyIDs.size());
	if(!deferredDRMRequestThreadStarted)
	{
		...
		{
			AAMPLOG_INFO(aamp->mConfig, "New Deferred DRM License thread starting");
	...
	{
		AAMPLOG_TRACE(aamp->mConfig, "Diferred License Request Thread already running");
	}
}
...
{
	AAMPLOG_INFO(aamp->mConfig, "Enter");
	int deferTime;
	bool exitLoop = false;
	// Start tread
	do
	{
		...
		if(mPendingKeyIDs.empty())
		{
			...
			{
				AAMPLOG_ERR(aamp->mConfig, "aborted");
				break;
			}
			else
			{
		...
		while(!mPendingKeyIDs.empty())
		{
			...
			if (mCommonKeyDuration > 0)
			{
				...
				AAMPLOG_TRACE(aamp->mConfig, "sleep over for deferred time:%d", deferTime);
			}

			if((aamp->DownloadsAreEnabled()) && (!mEarlyAvailableKeyIDMap[keyID].isLicenseFailed))
			{
				AAMPLOG_TRACE(aamp->mConfig, "Processing License request after sleep");
	...
	AAMPLOG_INFO(aamp->mConfig, "Exit");
}
#endif
...
int StreamAbstractionAAMP_MPD::GetBestAudioTrackByLanguage( int &desiredRepIdx, AudioType &CodecType)
{
	...
	for( int iAdaptationSet = 0; iAdaptationSet < numAdaptationSets; iAdaptationSet++)
	{
		...
		if( IsContentType(adaptationSet, eMEDIATYPE_AUDIO) )
		{
			...
			AAMPLOG_INFO(aamp->mConfig,  "track#%d score = %d\n", iAdaptationSet, score );
			if( score > bestScore )
			{
				bestScore = score;
...
void StreamAbstractionAAMP_MPD::StreamSelection( bool newTune, bool forceSpeedsChangedEvent)
{
	...
	if(!period)
	{
		AAMPLOG_WARN(aamp->mConfig, "period is null");  //CID:84742 - Null Returns
	}
	AAMPLOG_INFO(aamp->mConfig, "Selected Period index %d, id %s", mCurrentPeriodIdx, period->GetId().c_str());
	for( int i = 0; i < mMaxTracks; i++ )
	{
	...
	if (rate == AAMP_NORMAL_PLAY_RATE)
	{
		...
		if( audioAdaptationSet )
		{
			...
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: > lang[%s] AudioType[%d]", lang.c_str(), selectedCodecType);
		}
		else
		{
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Unable to get audioAdaptationSet.");
		}
	}
	...
	for (int i = 0; i < mMaxTracks; i++)
	{
		...
		{
			IAdaptationSet *adaptationSet = period->GetAdaptationSets().at(iAdaptationSet);
			AAMPLOG_TRACE(aamp->mConfig, "StreamAbstractionAAMP_MPD: > Content type [%s] AdapSet [%d] ", adaptationSet->GetContentType().c_str(), iAdaptationSet);
			if (IsContentType(adaptationSet, (MediaType)i ))
			{
				if (AAMP_NORMAL_PLAY_RATE == rate)
				{
					...
					{
						AAMPLOG_INFO(aamp->mConfig, "Checking subs - mime %s lang %s selAdaptationSetIndex %d",
						...
						if (!selectedTextTrack.index.empty())
						{
							if (IsMatchingLanguageAndMimeType((MediaType)i, selectedTextTrack.language, adaptationSet, selRepresentationIndex))
							{
								...
								AAMPLOG_INFO(aamp->mConfig, "adapSet Id %s selName %s", adapSetName.c_str(), selectedTextTrack.name.c_str());
					...
					else if (eMEDIATYPE_AUX_AUDIO == i && aamp->IsAuxiliaryAudioEnabled())
					{
						...
						if (aamp->GetAuxiliaryAudioLanguage() == aamp->preferredLanguagesString)
						{
							AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD:auxiliary audio same as primary audio, set forward audio flag");
				...
				else if ((!ISCONFIGSET(eAAMPConfig_AudioOnlyPlayback)) && (eMEDIATYPE_VIDEO == i))
				{
					...
					if ( IsIframeTrack(adaptationSet) )
					{
						AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Got TrickMode track");
				...
				if (eMEDIATYPE_AUDIO == i || eMEDIATYPE_SUBTITLE == i)
				{
					...
					for (int representationIndex = 0; representationIndex < representation.size(); representationIndex++)
					{
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Audio Track - lang:%s, group:%s, name:%s, codec:%s, bandwidth:%d",
								lang.c_str(), group.c_str(), name.c_str(), codec.c_str(), bandwidth);
							aTracks.push_back(AudioTrackInfo(index, lang, group, name, codec, bandwidth));
						}
						else
						{
							AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Text Track - lang:%s, isCC:0, group:%s, name:%s, codec:%s",
								lang.c_str(), group.c_str(), name.c_str(), codec.c_str());
						...
						if (schemeId.find("urn:scte:dash:cc:cea") != string::npos)
						{
							...
							if (!value.empty())
							{
								...
								while (delim != std::string::npos)
								{
									...
									AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: CC Track - lang:%s, isCC:1, group:%s, id:%s",
							...
							else
							{
								...
								AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: CC Track - group:%s, isCC:1", schemeId.c_str());
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Selected Audio Track codec is unknown");
		...
		if ((AAMP_NORMAL_PLAY_RATE == rate) && (pMediaStreamContext->enabled == false) && selAdaptationSetIndex >= 0)
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: AudioType Changed %d -> %d",
						aamp->previousAudioType, selectedCodecType);
				aamp->previousAudioType = selectedCodecType;
				SetESChangeStatus();
			}
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Media[%s] Adaptation set[%d] RepIdx[%d] TrackCnt[%d]\n",
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: No valid adaptation set found for Media[%s]\n",
				getMediaTypeName(MediaType(i)));
		}

		AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Media[%s] %s\n",
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Auxiliary enabled, but subtitle disabled, swap MediaStreamContext of both");
	...
	{ // what about audio+subtitles?
		if(newTune)
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Audio only period");
...
static void GetBitrateInfoFromCustomMpd( const IAdaptationSet *adaptationSet, std::vector<Representation *>& representations )
{
	...
	for(int i = 0; i < subNodes.size(); i ++)
	{
		...
		if(node != NULL)
		{
			if(node->GetName() == "AvailableBitrates")
			{
				...
				for(int reprIter = 0; reprIter < reprNodes.size(); reprIter++)
				{
					...
					{
						AAMPLOG_WARN_GP("reprNode  is null");  //CID:85171 - Null Returns
					}
				}
				break;
			}
		...
		{
			AAMPLOG_WARN_GP("node is null");  //CID:81731 - Null Returns
		}
	}
}
...
AAMPStatusType StreamAbstractionAAMP_MPD::UpdateTrackInfo(bool modifyDefaultBW, bool periodChanged, bool resetTimeLineIndex)
{
	...
	for (int i = 0; i < mNumberOfTracks; i++)
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "pMediaStreamContext  is null");  //CID:82464,84186 - Null Returns
		}
		...
		if(pMediaStreamContext->enabled)
		{
			...
			if (i == eMEDIATYPE_VIDEO)
			{
				if(isFogTsb && mUpdateStreamInfo)
				{
					...
					for (int idx = 0; idx < representationCount; idx++)
					{
						...
						if(representation != NULL)
						{
							...
							{
								AAMPLOG_INFO(aamp->mConfig, "Video Profile Ignoring resolution=%d:%d display=%d:%d Bw=%ld", mStreamInfo[idx].resolution.width, mStreamInfo[idx].resolution.height, aamp->mDisplayWidth, aamp->mDisplayHeight, mStreamInfo[idx].bandwidthBitsPerSecond);
								continue;
							}
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "representation  is null");  //CID:82489 - Null Returns
						}
					}
				...
				else if(!isFogTsb && mUpdateStreamInfo)
				{
					...
					if ((representationCount != GetProfileCount()) && mStreamInfo)
					{
						...
						AAMPLOG_WARN(aamp->mConfig, "representationIndex set to (-1) to find currentProfileIndex");
					}
					if (!mStreamInfo)
					{
						mStreamInfo = new StreamInfo[representationCount];
					...
					for (int pidx = 0; pidx < idx; pidx++)
					{
						...
						{
							AAMPLOG_INFO(aamp->mConfig, "Video Profile ignoring for resolution= %d:%d display= %d:%d BW=%ld", mStreamInfo[pidx].resolution.width, mStreamInfo[pidx].resolution.height, aamp->mDisplayWidth, aamp->mDisplayHeight, mStreamInfo[pidx].bandwidthBitsPerSecond);
						}
						else
						{
							if ((mStreamInfo[pidx].bandwidthBitsPerSecond > minBitrate) && (mStreamInfo[pidx].bandwidthBitsPerSecond < maxBitrate))
							{
								...
								AAMPLOG_INFO(aamp->mConfig, "Added Video Profile to ABR BW= %ld res= %d:%d display= %d:%d pc:%d", mStreamInfo[pidx].bandwidthBitsPerSecond, mStreamInfo[pidx].resolution.width, mStreamInfo[pidx].resolution.height, aamp->mDisplayWidth, aamp->mDisplayHeight, aamp->mProfileCappedStatus);
							}
						}
					}
					if(adaptationSets.size() > 0)
					...
					if (0 == addedProfiles)
					{
						...
						AAMPLOG_WARN(aamp->mConfig, "No video profiles found, minBitrate : %ld maxBitrate: %ld", minBitrate, maxBitrate);
						return ret;
					}
					if (modifyDefaultBW)
					{
			...
			if(-1 == pMediaStreamContext->representationIndex)
			{
				...
				{
					AAMPLOG_WARN(aamp->mConfig, "[WARN] !! representationIndex is '-1' override with '0' since Custom MPD has single representation");
			...
			if(!baseUrls)
			{
				AAMPLOG_WARN(aamp->mConfig, "baseUrls is null");  //CID:84531 - Null Returns
			}
			if (baseUrls->size() == 0)
			{
			...
			if( segmentTemplates.HasSegmentTemplate())
			{
				...
				AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: Track %d timeLineIndex %d fragmentDescriptor.Number %lu", i, pMediaStreamContext->timeLineIndex, pMediaStreamContext->fragmentDescriptor.Number);
			}
...
double StreamAbstractionAAMP_MPD::GetCulledSeconds()
{
	...
	if (pMediaStreamContext->adaptationSet)
	{
		...
		if(segmentTemplates.HasSegmentTemplate())
		{
				...
				while (iter1 < mMPDPeriodsInfo.size())
				{
					...
					if(prevPeriodInfo.periodId == currFirstPeriodInfo.periodId)
					{
						...
						if(currFirstPeriodInfo.startTime > prevPeriodInfo.startTime)
						{
							...
							AAMPLOG_INFO(aamp->mConfig, "PeriodId %s, prevStart %" PRIu64 " currStart %" PRIu64 " culled %f",
					...
					else
					{
						...
						iter1++;
						AAMPLOG_WARN(aamp->mConfig, " PeriodId %s , with last known duration %f seems to have got culled",
			...
			{
				AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: NULL segmentTimeline. Hence modifying culling logic based on MPD availabilityStartTime, periodStartTime, fragment number and current time");
				...
				if(firstSegTempate)
				{
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: newStartTimeSeconds %f mPrevStartTimeSeconds %F", newStartSegment, mPrevStartTimeSeconds);
					}
		...
		else
		{
			...
			if (segmentList)
			{
				...
				else
				{
					...
					for(int iPeriod = periods.size() - 1 ; iPeriod >= 0; iPeriod--)
					{
						...
						if(!adaptationSet)
						{
							AAMPLOG_WARN(aamp->mConfig, "adaptationSet is null");  //CID:80968 - Null Returns
						}
					...
					AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: PrevOffset %ld CurrentOffset %ld culled (%f)", mPrevLastSegurlOffset, currOffset, culled);
			...
			{
				AAMPLOG_INFO(aamp->mConfig, "StreamAbstractionAAMP_MPD: NULL segmentTemplate and segmentList");
			}
		}
	}
	else
	{
		AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: NULL adaptationset");
	}
	return culled;
}
...
void StreamAbstractionAAMP_MPD::FetchAndInjectInitialization(bool discontinuity)
{
				...
				if( segmentTemplates.HasSegmentTemplate() )
				{
					...
					if (!initialization.empty())
					{
						...
						if(!dlThreadCreated)
						{
							...
							if(ret != 0)
							{
								AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: pthread_create failed for TrackDownloader with errno = %d, %s", errno, strerror(errno));
								delete fetchParams;
							}
							else
							{
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "initialization  is null");  //CID:84853 ,86291- Null Retur
					}
				}
				else
				{
...
#ifdef DEBUG_TIMELINE
							AAMPLOG_WARN(aamp->mConfig, "init %s %d..%d", getMediaTypeName(pMediaStreamContext->mediaType), start, fin);
#endif
							std::string fragmentUrl;
							GetFragmentUrl(fragmentUrl, &pMediaStreamContext->fragmentDescriptor, "");
							if(pMediaStreamContext->WaitForFreeFragmentAvailable(0))
					...
					else
					{
						...
						if (segmentList)
						{
							...
							if( !urlType )
							{
								...
								if( !urlType )
								{
									AAMPLOG_WARN(aamp->mConfig, "initialization is null");
									continue;
								}
							}
							...
							else
							{
								...
								if (segmentURLs.size() > 0)
								{
									...
									if(firstSegmentURL != NULL)
									{
										...
										AAMPLOG_INFO(aamp->mConfig, "firstSegmentRange %s [%s]",
										...
										if (sscanf(firstSegmentRange, "%d-%d", &start, &fin) == 2)
										{
											...
											{
												AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: segmentList - cannot determine range for Initialization - first segment start %d",
														start);
											}
										}
									}
									...
									{
										AAMPLOG_WARN(aamp->mConfig, "firstSegmentURL is null");  //CID:80649 - Null Returns
									}
								}
#endif
								if (!range.empty())
								{
									std::string fragmentUrl;
									GetFragmentUrl(fragmentUrl, &pMediaStreamContext->fragmentDescriptor, "");
									AAMPLOG_INFO(aamp->mConfig, "%s [%s]", getMediaTypeName(pMediaStreamContext->mediaType),
											range.c_str());
									if(pMediaStreamContext->WaitForFreeFragmentAvailable(0))
									{
										pMediaStreamContext->profileChanged = false;
								...
								{
									AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: segmentList - empty range string for Initialization"
											);
								}
							}
						}
	...
	if(dlThreadCreated)
	{
		AAMPLOG_TRACE(aamp->mConfig, "Waiting for pthread_join trackDownloadThread");
		pthread_join(trackDownloadThreadID, NULL);
		AAMPLOG_TRACE(aamp->mConfig, "Joined trackDownloadThread");
		delete fetchParams;
	}
}
...
bool StreamAbstractionAAMP_MPD::CheckForInitalClearPeriod()
{
	...
	if(ret)
	{
		AAMPLOG_WARN(aamp->mConfig, "Initial period is clear period, trying work around");
	}
	return ret;
}
		...
		while(iPeriod < numPeriods && !encryptionFound)
		{
			...
			if(period != NULL)
			{
				...
				for(unsigned iAdaptationSet = 0; iAdaptationSet < numAdaptationSets && !encryptionFound; iAdaptationSet++)
				{
						...
						if (IsContentType(adaptationSet, (MediaType)i ))
						{
							...
							else
							{
								...
								else if (mAudioType != eAUDIO_UNKNOWN)
								{
									...
									AAMPLOG_WARN(aamp->mConfig, " Audio type %d", selectedAudioType);
								}
								else
								{
									AAMPLOG_WARN(aamp->mConfig, " Audio type eAUDIO_UNKNOWN");
								}
								...
								if(segmentTemplates.HasSegmentTemplate())
								{
									...
									if (!initialization.empty())
									{
										...
										{
											AAMPLOG_WARN(aamp->mConfig, "Pushing encrypted header for %s", getMediaTypeName(MediaType(i)));
											...
											if(!temp)
											{
												AAMPLOG_WARN(aamp->mConfig, "Error at  pthread_create");  //CID:84438 - checked return
											}
										}
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "adaptationSet is null");  //CID:84361 - Null Returns
					}
				}
			}
			else
			{
				AAMPLOG_WARN(aamp->mConfig, "period    is null");  //CID:86137 - Null Returns
			}
			iPeriod++;
		}
	}
			...
			{	// profile not changed and Cache not full scenario
				if (!pMediaStreamContext->eos)
				{
							...
							&& mCdaiObject->CheckForAdTerminate(pMediaStreamContext->fragmentTime - pMediaStreamContext->periodStartOffset))
					{
						...
						AAMPLOG_WARN(aamp->mConfig, "[CDAI] Adbreak ended early. Terminating Ad playback. fragmentTime[%lf] periodStartOffset[%lf]",
...
void StreamAbstractionAAMP_MPD::FetcherLoop()
{
	...
	if(!currPeriod)
	{
		AAMPLOG_WARN(aamp->mConfig, "currPeriod is null");  //CID:80891 - Null Returns
	}
	std::string currentPeriodId = currPeriod->GetId();
	mPrevAdaptationSetCount = currPeriod->GetAdaptationSets().size();
	AAMPLOG_WARN(aamp->mConfig, "aamp: ready to collect fragments. mpd %p", mpd);
	do
	{
		...
		if (mpd)
		{
			...
			AAMPLOG_INFO(aamp->mConfig, "MPD has %zu periods current period index %u", numPeriods, mCurrentPeriodIdx);
			...
			while(iPeriod < numPeriods && !exitFetchLoop)  //CID:95090 - No effect
			{
				...
				if (periodChanged || mpdChanged || adStateChanged)
				{
							...
							for (auto tempPeriod : vssPeriods)
							{
								if (NULL != tempPeriod)
								{
									...
									if (!keyIdArray.empty())
									{
										...
										AAMPLOG_INFO(aamp->mConfig, "New VSS Period : %s Key ID: %s", tempPeriod->GetId().c_str(), keyIdDebugStr.c_str());
										...
										{
											AAMPLOG_TRACE(aamp->mConfig, "Skipping license request for keyID : %s", keyIdDebugStr.c_str() );
										}
									}
									else
									{
										AAMPLOG_WARN(aamp->mConfig, "Failed to get keyID for vss common key EAP");
									}
									}
									else
									{
										AAMPLOG_ERR(aamp->mConfig, "Failed to Create DRM Helper");	
									}
								}
							}
					...
					if (periodChanged)
					{
						...
						//for VOD and cDVR
						AAMPLOG_WARN(aamp->mConfig, " Period(%s - %d/%zu) Offset[%lf] IsLive(%d) IsCdvr(%d) ",
					...
					if(adStateChanged && AdState::OUTSIDE_ADBREAK == mCdaiObject->mAdState)
					{
						...
						for(iPeriod=0;iPeriod < numPeriods;  iPeriod++)
						{
							...
							{
								mCurrentPeriodIdx = iPeriod;
								AAMPLOG_INFO(aamp->mConfig, "[CDAI] Landed at the periodId[%d] ",mCurrentPeriodIdx);
								break;
							}
						}
					}
					...
					if(currentPeriodId != mCurrentPeriod->GetId())
					{
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "Period ID changed from \'%s\' to \'%s\' [BasePeriodId=\'%s\']", currentPeriodId.c_str(),mCurrentPeriod->GetId().c_str(), mBasePeriodId.c_str());
							currentPeriodId = mCurrentPeriod->GetId();
							mPrevAdaptationSetCount = adaptationSetCount;
							periodChanged = true;
							requireStreamSelection = true;
							AAMPLOG_WARN(aamp->mConfig, "playing period %d/%d", iPeriod, (int)numPeriods);
						}
						else
						{
							for (int i = 0; i < mNumberOfTracks; i++)
							...
							AAMPLOG_WARN(aamp->mConfig, "Period ID not changed from \'%s\' to \'%s\',since period is empty [BasePeriodId=\'%s\']", currentPeriodId.c_str(),mCurrentPeriod->GetId().c_str(), mBasePeriodId.c_str());
						}
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "Change in AdaptationSet count; adaptationSetCount %d  mPrevAdaptationSetCount %d,updating stream selection", adaptationSetCount, mPrevAdaptationSetCount);
						mPrevAdaptationSetCount = adaptationSetCount;
						requireStreamSelection = true;
					}
					else
					{
						for (int i = 0; i < mNumberOfTracks; i++)
						{
							...
							{
								AAMPLOG_WARN(aamp->mConfig, "AdaptationSet index changed; updating stream selection");
								requireStreamSelection = true;
							}
						}
					}
					...
					if(mIsLiveStream || lastLiveFlag)
					{
						...
						if(culled > 0)
						{
							AAMPLOG_INFO(aamp->mConfig, "Culled seconds = %f", culled);
						...
						{
							AAMPLOG_INFO(aamp->mConfig, "[CDAI]: Resuming channel playback at PeriodID[%s] at Position[%lf]", currentPeriodId.c_str(), mCdaiObject->mContentSeekOffset);
							...
							(ISCONFIGSET(eAAMPConfig_MPDDiscontinuityHandlingCdvr) || (!aamp->IsInProgressCDVR())))
					{
						...
						{
							AAMPLOG_ERR(aamp->mConfig, " Error! Audio or Video track missing in period, ignoring discontinuity");
						}
						else
						{
							...
							if( segmentTemplates.HasSegmentTemplate() )
							{
								...
								{
									AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: discontinuity detected nextSegmentTime %" PRIu64 " FirstSegmentStartTime %" PRIu64 " ", nextSegmentTime, segmentStartTime);
									discontinuity = true;
									mFirstPTS = (double)segmentStartTime/(double)segmentTemplates.GetTimescale();
								}
								else
								{
									AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: No discontinuity detected nextSegmentTime %" PRIu64 " FirstSegmentStartTime %" PRIu64 " ", nextSegmentTime, segmentStartTime);
								}
							}
							else
							{
				...
				while (!exitFetchLoop && !liveMPDRefresh)
				{
					...
					if (vEos || aEos)
					{
						...
						if(vEos && aEos)
						{
							AAMPLOG_INFO(aamp->mConfig, "EOS - Exit fetch loop ");
							if(AdState::IN_ADBREAK_AD_PLAYING == mCdaiObject->mAdState)
							{
					...
					if (AdState::OUTSIDE_ADBREAK != mCdaiObject->mAdState)
					{
						...
						{
							AAMPLOG_INFO(aamp->mConfig, "[CDAI]: BasePeriod[%s] completed @%lf. Changing to next ", mBasePeriodId.c_str(),mBasePeriodOffset);
							break;
						}
					...
					else if(bCacheFullState)
					{
						...
						int timeoutMs = 200;
						AAMPLOG_TRACE(aamp->mConfig, "Cache full state,no download until(%d) Time(%lld)",timeoutMs,aamp_GetCurrentTimeMS());
						bool temp =  mMediaStreamContext[eMEDIATYPE_VIDEO]->WaitForFreeFragmentAvailable(timeoutMs);
						if(temp == false)
						{
							AAMPLOG_TRACE(aamp->mConfig, "Waiting for FreeFragmentAvailable");  //CID:82355 - checked return
						}
					}
					else
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: - null mpd");
		}
		...
		if (!liveMPDRefresh && mLastPlaylistDownloadTimeMs)
		{
			...
			else
			{
				...
				// limit the logs when buffer is low
				{
					...
					if((bufferlowCnt++ & 5) == 0)
					{
						AAMPLOG_WARN(aamp->mConfig, "Buffer is running low(%ld).Refreshing playlist(%d).PlayPosition(%lld) End(%lld)",
			...
			AAMPLOG_INFO(aamp->mConfig, "aamp playlist end refresh bufferMs(%ld) delay(%d) delta(%d) End(%lld) PlayPosition(%lld)",
		...
		if(mIsLiveStream)
		{
			...
			mFirstPeriodStartTime = GetPeriodStartTime(mpd,0);
			AAMPLOG_INFO(aamp->mConfig, "Updating period index after mpd refresh, mFirstPeriodStartTime: %lf", mFirstPeriodStartTime);
		}
		else
		{
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "MPD Fragment Collector detected reset in Period(New Size:%zu)(currentIdx:%d->%zu)",
	...
	while (!exitFetchLoop);
	AAMPLOG_WARN(aamp->mConfig, "MPD fragment collector done");
}
...
bool StreamAbstractionAAMP_MPD::CheckForVssTags()
{
	...
	if (!nodePtr)
	{
		AAMPLOG_ERR(aamp->mConfig, "API Failed due to Invalid Arguments");
	}
	else
	{
				...
				if (childNode->HasAttribute("schemeIdUri"))
				{
					...
					if (schemeIdUri == schemeIdUriVss)
					{
						if (childNode->HasAttribute("value"))
						{
							...
							AAMPLOG_INFO(aamp->mConfig, "Recieved Common Key Duration : %d of VSS stream", mCommonKeyDuration);
							isVss = true;
						}
					}
#endif
					...
					if ((subNodeName == "ContentIdentifier") && (infoNodeType == "URI" || infoNodeType == "URN"))
					{
						if (infoNode->HasAttribute("value"))
						{
							...
							if (value.find(VSS_VIRTUAL_STREAM_ID_PREFIX) != std::string::npos)
							{
								...
								AAMPLOG_INFO(aamp->mConfig, "Parsed Virtual Stream ID from manifest:%s", ret.c_str());
								break;
							}
						}
					}
...
void StreamAbstractionAAMP_MPD::Stop(bool clearChannelData)
{
	...
	{
		AAMPLOG_INFO(aamp->mConfig, "Waiting to join CreateDRMSession thread");
		int rc = pthread_join(createDRMSessionThreadID, NULL);
		if (rc != 0)
		{
			AAMPLOG_WARN(aamp->mConfig, "pthread_join returned %d for createDRMSession Thread", rc);
		}
		AAMPLOG_INFO(aamp->mConfig, "Joined CreateDRMSession thread");
		drmSessionThreadStarted = false;
	}
						...
						for (int repIndex = 0; repIndex < representation.size(); repIndex++)
						{
							...
							for (unsigned i = 0; i < subnodes.size() && !done; i++)
							{
								...
								if(xml != NULL)
								{
									if (xml->GetName() == "EssentialProperty")
									{
										...
										if(xml->HasAttribute("value"))
										{
											...
											if(!value.empty())
											{
												...
												AAMPLOG_WARN_GP(" - value=%dx%d",w,h);
												done = true;
											}
										}
									}
									...
									{
										AAMPLOG_WARN_GP(" - skipping name %s", xml->GetName().c_str());
									}
								}
								else
								{
...
std::vector<ThumbnailData> StreamAbstractionAAMP_MPD::GetThumbnailRangeData(double tStart, double tEnd, std::string *baseurl, int *raw_w, int *raw_h, int *width, int *height)
{
	...
	if(indexedTileInfo.empty())
	{
		...
		{
			AAMPLOG_WARN(aamp->mConfig, "calling indexthumbnail");
			deIndexTileInfo(indexedTileInfo);
			indexThumbnails(mpd, aamp->mthumbIndexValue, indexedTileInfo, thumbnailtrack);
		}
		else
		{
			AAMPLOG_WARN(aamp->mConfig, "Exiting. Thumbnail track not configured!!!.");
			return data;
		}
	}
		...
		for(auto &event: eventStream->GetEvents())
		{
			...
			if(event && (!mIsLiveManifest || 0 != event->GetDuration()))
			{
				for(auto &evtChild: event->GetAdditionalSubNodes())
				{
						...
						if(prefix+"Signal" == evtChild->GetName())
						{
							for(auto &signalChild: evtChild->GetNodes())
							{
								if(signalChild && prefix+"Binary" == signalChild->GetName())
								{
									...
									{
										AAMPLOG_WARN(aamp->mConfig, "[CDAI]: Found a scte35:Binary in manifest with empty binary data!!");
									}
								}
								else
								{
									AAMPLOG_WARN(aamp->mConfig, "[CDAI]: Found a scte35:Signal in manifest without scte35:Binary!!");
								}
							}
						}
					}
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "evtChild is null");  //CID:85816 - Null Return
					}
				}
			}
		}
	...
	switch(mCdaiObject->mAdState)
	{
		case AdState::OUTSIDE_ADBREAK:
			if(AdEvent::DEFAULT == evt || AdEvent::INIT == evt)
			{
				...
				if(!brkId.empty())
				{
					AAMPLOG_INFO(aamp->mConfig, "[CDAI] CheckForAdStart found Adbreak. adIdx[%d] mBasePeriodOffset[%lf] adOffset[%lf].", adIdx, mBasePeriodOffset, adOffset);
					...
					if(-1 != adIdx && mCdaiObject->mAdBreaks[brkId].ads)
					{
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "[CDAI]: STARTING ADBREAK[%s] AdIdx[%d] Found at Period[%s].", brkId.c_str(), adIdx, mBasePeriodId.c_str());
						...
						{
							AAMPLOG_WARN(aamp->mConfig, "[CDAI]: AdIdx[%d] in the AdBreak[%s] is invalid. Skipping.", adIdx, brkId.c_str());
						}
					...
					if(AdState::IN_ADBREAK_AD_PLAYING != mCdaiObject->mAdState)
					{
						AAMPLOG_WARN(aamp->mConfig, "[CDAI]: BasePeriodId in Adbreak. But Ad not available. BasePeriodId[%s],Adbreak[%s]", mBasePeriodId.c_str(), brkId.c_str());
		...
		case AdState::IN_ADBREAK_AD_NOT_PLAYING:
			if(AdEvent::BASE_OFFSET_CHANGE == evt || AdEvent::PERIOD_CHANGE == evt)
			{
				...
				if(-1 != adIdx && mCdaiObject->mAdBreaks[brkId].ads)
				{
					...
					if(!(mCdaiObject->mAdBreaks[brkId].ads->at(adIdx).invalid))
					{
						AAMPLOG_WARN(aamp->mConfig, "[CDAI]: AdIdx[%d] Found at Period[%s].", adIdx, mBasePeriodId.c_str());
						mCdaiObject->mCurAds = mCdaiObject->mAdBreaks[brkId].ads;
				...
				{
					AAMPLOG_WARN(aamp->mConfig, "[CDAI]: ADBREAK[%s] ENDED. Playing the basePeriod[%s].", mCdaiObject->mCurPlayingBreakId.c_str(), mBasePeriodId.c_str());
		...
		case AdState::IN_ADBREAK_AD_PLAYING:
			...
			{
				AAMPLOG_WARN(aamp->mConfig, "[CDAI]: Ad finished at Period. Waiting to catchup the base offset.[idx=%d] [period=%s]", mCdaiObject->mCurAdIdx, mBasePeriodId.c_str());
			...
			else if(AdEvent::AD_FAILED == evt)
			{
				...
				AAMPLOG_WARN(aamp->mConfig, " [CDAI]: Ad Playback failed. Going to the base period[%s] at offset[%lf].Ad[idx=%d]", mBasePeriodId.c_str(), mBasePeriodOffset,mCdaiObject->mCurAdIdx);
		...
		case AdState::IN_ADBREAK_WAIT2CATCHUP:
			...
			{
				AAMPLOG_WARN(aamp->mConfig, " [CDAI]: BUG! BUG!! BUG!!! We should not come here.AdIdx[-1].");
				mCdaiObject->mCurPlayingBreakId = "";
				mCdaiObject->mCurAds = nullptr;
				mCdaiObject->mCurAdIdx = -1;
			...
			if(AdEvent::DEFAULT == evt)
			{
				...
				AAMPLOG_WARN(aamp->mConfig, "[CDAI]: Current Ad placement Completed. Ready to play next Ad.");
		...
		case AdState::IN_ADBREAK_AD_READY2PLAY:
			if(AdEvent::DEFAULT == evt)
			{
				...
				if(mCdaiObject->mCurAdIdx >= 0 && mCdaiObject->mCurAdIdx < mCdaiObject->mCurAds->size())
				{
					...
					{
						AAMPLOG_WARN(aamp->mConfig, "[CDAI]: AdIdx is invalid. Skipping. AdIdx[%d].", mCdaiObject->mCurAdIdx);
						mCdaiObject->mAdState = AdState::IN_ADBREAK_AD_NOT_PLAYING;
					}
					else
					{
						AAMPLOG_WARN(aamp->mConfig, "[CDAI]: Next AdIdx[%d] Found at Period[%s].", mCdaiObject->mCurAdIdx, mBasePeriodId.c_str());
						mCdaiObject->mAdState = AdState::IN_ADBREAK_AD_PLAYING;
				...
				else
				{
					...
					AAMPLOG_WARN(aamp->mConfig, "[CDAI]: All Ads in the ADBREAK[%s] FINISHED. Playing the basePeriod[%s] at Offset[%lf].", mCdaiObject->mCurPlayingBreakId.c_str(), mBasePeriodId.c_str(), mCdaiObject->mContentSeekOffset);
	...
	if(stateChanged)
	{
		...
		if(AdState::IN_ADBREAK_AD_PLAYING == mCdaiObject->mAdState)
		{
			...
			if(NULL == adNode.mpd)
			{
				...
				if(NULL == adNode.mpd)
				{
					AAMPLOG_WARN(aamp->mConfig, "[CDAI]: Ad playback failed. Not able to download Ad manifest from FOG.");
					mCdaiObject->mAdState = AdState::IN_ADBREAK_AD_NOT_PLAYING;
		...
		if(stateChanged)
		{
			AAMPLOG_WARN(aamp->mConfig, "[CDAI]: State changed from [%s] => [%s].", ADSTATE_STR[static_cast<int>(oldState)],ADSTATE_STR[static_cast<int>(mCdaiObject->mAdState)]);
		}
...
bool StreamAbstractionAAMP_MPD::IsMatchingLanguageAndMimeType(MediaType type, std::string lang, IAdaptationSet *adaptationSet, int &representationIndex)
{
	   ...
	   AAMPLOG_INFO(aamp->mConfig, "type %d inlang %s current lang %s", type, lang.c_str(), adapLang.c_str());
	   if (adapLang == lang)
	   {
			   ...
			   if (ret != true)
			   {
					   ...
					   AAMPLOG_WARN(aamp->mConfig, "StreamAbstractionAAMP_MPD: Found matching track[%d] with language:%s but not supported mimeType and thus disabled!!\n",
											   type, lang.c_str());
			   }
	   }
	   return ret;
