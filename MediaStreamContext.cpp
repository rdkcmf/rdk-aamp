...
void MediaStreamContext::InjectFragmentInternal(CachedFragment* cachedFragment, bool &fragmentDiscarded)
{
    ...
    {
        AAMPLOG_INFO("%s:%d Full Fragment Send Ignored in LL Mode", __FUNCTION__, __LINE__);
    }
    ...
    , bool playingAd)
{
    ...
    else
    {
        ...
        ret = aamp->LoadFragment(this, bucketType, fragmentUrl,effectiveUrl, &cachedFragment->fragment, curlInstance,
...
{
#ifdef AAMP_DEBUG_INJECT
    logprintf("%s:%d [%s] Chunk Buffer Length %d Remote URL %s", __FUNCTION__, __LINE__,name, size, remoteUrl.c_str());
#endif
    CachedFragmentChunk* cachedFragmentChunk = NULL;
    cachedFragmentChunk = GetFetchChunkBuffer(true);
    if(NULL == cachedFragmentChunk)
    {
            logprintf("%s:%d [%s] Something Went wrong - Can't get FetchChunkBuffer",__FUNCTION__, __LINE__,name);
            return false;
    }
    cachedFragmentChunk->type = actualType;
    aamp_AppendBytes(&cachedFragmentChunk->fragmentChunk, ptr, size);
#ifdef AAMP_DEBUG_INJECT
    logprintf("%s:%d [%s] - cachedFragmentChunk %p ptr %p", __FUNCTION__, __LINE__,
