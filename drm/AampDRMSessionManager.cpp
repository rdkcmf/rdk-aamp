...
bool AampDRMSessionManager::IsKeyIdUsable(std::vector<uint8_t> keyIdArray)
{
	...
	for (int sessionSlot = 0; sessionSlot < mMaxDRMSessions; sessionSlot++)
	{
		...
		{
			AAMPLOG_INFO("Session created/inprogress at slot %d", sessionSlot);
...
int SpawnDRMLicenseAcquireThread(PrivateInstanceAAMP *aamp, DrmSessionDataInfo* drmData)
{
	...
	do{
		...
		AAMPLOG_INFO("Creating thread with sessionData = 0x%08x", drmData->sessionData );
		...
		{
			AAMPLOG_ERR("pthread_create failed for CreateDRMSession : error code %d, %s", errno, strerror(errno));
		}
	}while(0);
