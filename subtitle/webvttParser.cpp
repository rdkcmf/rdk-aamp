...
bool WebVTTParser::init(double startPos, unsigned long long basePTS)
{
	...
	logprintf("WebVTTParser::%s %d startPos:%.3f and mStartPTS:%lld\n", __FUNCTION__, __LINE__, startPos, mStartPTS);
...
bool WebVTTParser::processData(char* buffer, size_t bufferLen, double position, double duration)
{
	...
	traceprintf("WebVTTParser::%s %d Enter with position:%.3f and duration:%.3f \n", __FUNCTION__, __LINE__, position, duration);

	if (mReset)
	{
		mStartPos = (position * 1000) - mProgressOffset;
		mReset = false;
		logprintf("WebVTTParser::%s %d Received first buffer after reset with mStartPos:%.3f\n", __FUNCTION__, __LINE__, mStartPos);
	}
	...
	if (ret)
	{
		while (buffer)
		{
			...
			if (strstr(buffer, "X-TIMESTAMP-MAP") != NULL && mPtsOffset == 0)
			{
				...
				logprintf("Parsed local time:%lld and PTS:%lld and cuePTSOffset:%lld\n", localTime, mpegTime, mPtsOffset);
			}
			else if (strstr(buffer, " --> ") != NULL)
			{
				...
				while (token != NULL)
				{
					traceprintf("Inside tokenizer, token:%s\n", token);
...
bool WebVTTParser::close()
{
	...
	if (mVttQueueIdleTaskId != 0)
	{
                logprintf("WebVTTParser::%s %d Remove mVttQueueIdleTaskId %d\n", __FUNCTION__, __LINE__, mVttQueueIdleTaskId);
...
void WebVTTParser::sendCueData()
{
	...
	if (!mVttQueue.empty())
	{
		while(mVttQueue.size() > 0)
		{
			...
			{
				logprintf("Discarding cue with start:%.3f and text:%s\n", cue->mStart/1000.0, cue->mText.c_str());
			}
			delete cue;
		}
	}
